...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template name="FundRankingSelectedAddtionalCriteriaTemplate">
			...
			<fo:flow flow-name="xsl-region-body">
				...
				<fo:block id="FundRankingId">
					...
					<fo:block keep-with-next="always">
						<fo:marker marker-class-name="firstpage">
							...
							<fo:block space-before="7px" xsl:use-attribute-sets="legend_icon_text_style" start-indent="46px">
								...
								<xsl:if test="$showContractFundIcon != ''">
									<fo:external-graphic padding-after="-5px" content-height="15px" content-width="15px">
										<xsl:attribute name="src">
								...
								<xsl:if test="$showClosedToNBIconSelectedOnly = 'yes'">
									<fo:external-graphic padding-after="-5px" content-height="15px" content-width="15px">
										<xsl:attribute name="src">
				...
				<fo:block space-before="18px">
					...
					<fo:table space-after="18px" table-layout="fixed" width="100%">
						...
						<xsl:for-each select="criteriaSelections/criterions/criterion[1]/criteria">
						...
						<fo:table-body>
							...
							<xsl:for-each select="customization/selectedAssetClasses/assetClasses/assetClass">
									...
									<fo:table-row height="10px">
										<fo:table-cell>
											...
											<xsl:attribute name="number-columns-spanned">
												<xsl:if test="$criteriaCount_var = 1">7</xsl:if>
												<xsl:if test="$criteriaCount_var = 2">10</xsl:if>
												<xsl:if test="$criteriaCount_var = 3">13</xsl:if>
												<xsl:if test="$criteriaCount_var = 4">16</xsl:if>
												<xsl:if test="$criteriaCount_var = 5">19</xsl:if>
												<xsl:if test="$criteriaCount_var = 6">22</xsl:if>
											</xsl:attribute>
														...
														<xsl:for-each select="ancestor::iEvaluatorReport/fundDetails/fund[@id=$test_var and @isChecked='yes']">
															...
															<fo:table-row display-align="center">
																...
																<xsl:variable name="shortName_var">
																	...
																	<xsl:attribute name="background-color">#F5E4C2</xsl:attribute>	
																		<fo:block text-align="center" xsl:use-attribute-sets="fund_ranking_fund_detail">
																		<xsl:value-of select="ancestor::iEvaluatorReport/fundDetails/fund[@id=$test_var and @isChecked='yes']/fundMetrics/rankings/criteria[@shortname=$shortName_var]/result/@measure" />
																		</fo:block>
																	</fo:table-cell>
																	<fo:table-cell padding-start="0.1cm" xsl:use-attribute-sets="solid_blue_border_1BottomSide_fundrank">
																	<xsl:attribute name="background-color">#F5E4C2</xsl:attribute>	
																		<fo:block text-align="center" xsl:use-attribute-sets="fund_ranking_fund_detail">
																		    <xsl:value-of select="ancestor::iEvaluatorReport/fundDetails/fund[@id=$test_var and @isChecked='yes']/fundMetrics/rankings/criteria[@shortname=$shortName_var]/result/@rank" />
																		</fo:block>
																	</fo:table-cell>
													...
													<xsl:if test="$isLastAssetClassID_var != 'yes'">
														<fo:table-row height="13px" keep-with-previous="always">
															...
															<fo:table-cell border-start-color="#FFFFFF" border-start-width="0.2px" border-start-style="solid">
																...
																<xsl:if test="$criteriaCount_var = 1">3</xsl:if>
																<xsl:if test="$criteriaCount_var = 2">6</xsl:if>
																<xsl:if test="$criteriaCount_var = 3">9</xsl:if>
																<xsl:if test="$criteriaCount_var = 4">12</xsl:if>
																<xsl:if test="$criteriaCount_var = 5">15</xsl:if>
																<xsl:if test="$criteriaCount_var = 6">18</xsl:if>
								...
								<!--New End loop here to loop over all Asset Classes of Equity-->	
								</xsl:for-each>
							...
							<!--New End loop here to loop over all Asset Houses-->
						</fo:table-body>
					</fo:table>
				</fo:block>			
			</fo:flow>
