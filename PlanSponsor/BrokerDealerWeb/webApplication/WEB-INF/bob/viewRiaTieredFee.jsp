...
<content:contentBean contentId="<%=BDContentConstants.RIA_TIERED_FEE_TILTLE%>" 
   type="<%=BDContentConstants.TYPE_LAYOUT_PAGE%>" id="riaTieredFeeTitle" />
		...
		<ul class="proposal_nav_menu">
			<li>
				<a id="Requested_Printed_copies_sec" class="selected_link">
					...
					<content:getAttribute id="layoutPageBean" attribute="name" beanName="riaTieredFeeTitle" /> 
					</span>
				</a>
			</li>
			<div
...
<c:forEach items="${blockOfBusinessForm.riaFeeDetailsVO}" var="riaFeeDetailsList" >
				...
				<tr class="spec">
					<td colspan="4" class="name"><strong>Tiered Rate (previous month end)</strong></td>
					<c:choose>
						<c:when test="${(empty riaFeeDetailsList.riaTieredRate) or (riaFeeDetailsList.riaTieredRate lt 0)}">
							<td colspan="5" class="name"> - </td>
						</c:when>
						<c:otherwise>
							<td colspan="5" class="name"><report:number
									property="riaFeeDetailsList.riaTieredRate"
			...
			<tr class="spec">
				<td colspan="4" class="name"><strong>Tier Information</strong></td>
				<td colspan="5" class="name">&nbsp;</td>
			</tr>
			<tr>
...
<c:if test="${empty blockOfBusinessForm.riaFeeRangeVO}">
				<tr>
					<td colspan="5">No Tier Information available</td>
				</tr>
</c:if>
