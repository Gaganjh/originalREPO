...
<c:if test='${fapForm.messagesExist}'>
	{
		"errors": [
			{
				"messages" : "<report:formatMessages scope="request" escapeSequence="HTML"/>"
			}
		]
...
<c:choose>
	<c:when test="${fapForm.filterResultsId =='contractSearchResults'}"> 
		
		<c:if test='${!fapForm.messagesExist}'>
		{
		</c:if>
			
			"resultsLength" : "${fapForm.contractResultsSize}",
		
			"optionsList" : [
				<c:forEach var="contractResultsList" items="${fapForm.contractResultsList}" varStatus="contractResultsListStatus">
					{
						"label" : "${contractResultsList.label}",
						"value" : "${contractResultsList.value}"
					}
				<c:if test="${!contractResultsListStatus.last}">
					,
				</c:if> 
				</c:forEach>
			]
	...
	<c:when test="${fapForm.filterResultsId =='fundInfoAndSelectQueryOptions'}"> 
		{
			"baseFundArray" : [
				<c:forEach var="fund" items="${fundValueObject}"  varStatus="fundListStatus">
					{ 
						...
						"contractSelected"	: "${fund.selectedBycontract}"
					}
			...
			"jhiIndicatorFlg" : ${fapForm.jhiIndicatorFlg},
			"svpIndicatorFlg" : ${fapForm.svpIndicatorFlg}
		}
	</c:when>
...
'${innerOptions.key}" :[
					...
					<c:forEach var="element" items="${innerOptions.value}" varStatus="valueStatus">
						{
"label" : '${element.label}",
"value" : "${element.value}" } <c:if test="${!valueStatus.last}">
   ...
   <c:when test="${fapForm.filterResultsId =='reports'}"> 
		{
			<c:choose>
				...
				<c:if test="${fapForm.cofidDistChannel != null }">
					,	"cofidDistChannel" : "${fapForm.cofidDistChannel}"
					</c:if> 
		}
	</c:when>
