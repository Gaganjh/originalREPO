...
@SessionAttributes({"bdFundCheckLevel2Form"})

public class BDFundCheckLevel2Controller extends BDFundCheckBaseController {

	@ModelAttribute("bdFundCheckLevel2Form")
	public BDFundCheckLevel2Form populateForm() {
		return new BDFundCheckLevel2Form();
	}
	...
	static {
		forwards.put(INPUT, "/fundcheck/fundCheckL2.jsp");
		forwards.put(DEFAULT, "/fundcheck/fundCheckL2.jsp");
		forwards.put(SEARCHPDF, "/fundcheck/fundCheckL2.jsp");
...
//TODO SSA it is better to have this compare part of the FundCheckDocumentInfo object, in its equals method		
	 ...
	 */
	@RequestMapping(value ="/fundCheckL2",  method =  RequestMethod.GET) 
	public String doDefault(@Valid @ModelAttribute("bdFundCheckLevel2Form") BDFundCheckLevel2Form actionForm, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		...
		BDFundCheckLevel2Form fundCheckL2Form = (BDFundCheckLevel2Form) actionForm;
		fundCheckL2Form.setSearchType("");
		fundCheckL2Form.setSearchInput("");
		fundCheckL2Form.setFundCheckPDFAvailableForSearch(true);
		fundCheckL2Form.setShowSearchResults(false);
		fundCheckL2Form.setNoSearchResults(false);
    	fundCheckL2Form.setAddMoreFilters(false);
    	fundCheckL2Form.setNameMap(null);
    	fundCheckL2Form.setShowCompanyId(false);
		try {
			
			List resultProducerCodes = getProducerCodes(fundCheckL2Form, request);
			...
			setFundCheckInfo( fundCheckL2Form, request,producerCodes);
			BDFundCheckHelper bdFundCheckHelper = new BDFundCheckHelper();
			fundCheckL2Form.setSearchTypeMap(bdFundCheckHelper.getLevel2SearchTypeMap());
		} catch (ServiceUnavailableException sue) {
			logger.error("EReports service not available.", sue);
			request.setAttribute(BDConstants.TECHNICAL_DIFFICULTIES, true);
			return forwards.get(fundCheckL2Form.getAction());
		...
		String forward=forwards.get(fundCheckL2Form.getAction());
		if (logger.isDebugEnabled())
	 ...
	 */
	@RequestMapping(value ="/fundCheckL2" ,params={"action=searchPDF"} , method =  RequestMethod.POST) 
	public String doSearchPDF (@Valid @ModelAttribute("bdFundCheckLevel2Form") BDFundCheckLevel2Form actionForm, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
			if (errDirect != null) {
	 ...
	 * @throws SystemException
	 */
	@RequestMapping(value ="/fundCheckL2", params={"action=openPDF"} , method =  RequestMethod.POST) 
	    public String doOpenPDF (@Valid @ModelAttribute("bdFundCheckLevel2Form") BDFundCheckLevel2Form actionForm, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	    throws IOException,ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
			if (errDirect != null) {
	...
	private boolean doValidateSearch(AutoForm actionForm, 
		...
		if(StringUtils.isEmpty(fundCheckL2Form.getSearchType()) || StringUtils.isEmpty(fundCheckL2Form.getSearchInput())){
	        /**
		...
		}else if(CONTRACT_NAME_TEXT.equalsIgnoreCase(fundCheckL2Form.getSearchType())){
	        /**
	         * 2. Validate the Contract Name filter value is not numeric 
	         * and not less than 3 Characters.
	         */
			if (StringUtils.trimToNull(fundCheckL2Form.getSearchInput()) != null
	                && (StringUtils.trimToNull(fundCheckL2Form.getSearchInput()).length() < 3)) {
	            GenericException exception = new GenericException(
	                    BDErrorCodes.ENTER_ATLEAST_THREE_CHARS);
	            errorMessages.add(exception);
			}
		}else if(CONTRACT_NUMBER_TEXT.equalsIgnoreCase(fundCheckL2Form.getSearchType())){
			if(!StringUtils.isNumeric(fundCheckL2Form.getSearchInput())){
				fundCheckL2Form.setNoSearchResults(true);
			}
		}
		if(errorMessages.size() > 0){	
			setErrorsInRequest(request, errorMessages);
			isValidSearchCriteria=false;
	   	}else if(fundCheckL2Form.isNoSearchResults() == true){
	   		isValidSearchCriteria=false;
	   	}
		if (logger.isDebugEnabled()){
	...
	private void setFundCheckInfo(
			BDFundCheckLevel2Form fundCheckL2Form, HttpServletRequest request,List<String> producerCodes)
		...
		if (producerCodes != null && producerCodes.size() != 0) {
			...
			FundCheckDocumentInfo[] fundCheckArray = fundCheckResponse
					.getFundCheckDocList();
			if (fundCheckArray.length > 0) {
				fundCheckL2Form.setFundCheckPDFAvailable(true);
				...
				if(producerCodes.size() >= 2){
					fundCheckL2Form.setShowProducerCode(true);
				}
				...
				if (fundCheckArray.length >= 2) {
					...
					for (int i = 0; i < fundCheckArray.length; i++) {
						...
						if(USAFlag && NYFlag){
							fundCheckL2Form.setShowCompanyId(true);
							break;
						}
					}
				}
				//create Rows Of FundCheckInfo and set in formBean
				fundCheckL2Form.setFundCheckInfo(createRowsOfFundCheckInfo(fundCheckArray));
			}
		}
		if (logger.isDebugEnabled()) {
	...
	private List getProducerCodes(BDFundCheckLevel2Form actionForm,
		...
		if(user != null) {
			...
			if (profile!=null && (profile.getRoleType().compareTo(BDUserRoleType.FinancialRep) == 0)) {
	...
	protected FundCheckDocFileResponse getFundCheckPDF(
		...
		BDFundCheckLevel2Form fundCheckL2Form = (BDFundCheckLevel2Form) form;
		FundCheckDocumentInfo info = new FundCheckDocumentInfo();		
		String selectedPDF = fundCheckL2Form.getSelectedPDF();
		String selectedPDFCode = StringUtils.trim(String.valueOf(fundCheckL2Form
				.getSelectedPDFCode()));
		String season = StringUtils.trim(fundCheckL2Form.getSelectedSeason());
		String year = StringUtils.trim(fundCheckL2Form.getSelectedYear());
		String companyId = StringUtils.trim(fundCheckL2Form.getSelectedCompanyId());
		String language = StringUtils.trim(fundCheckL2Form.getSelectedLanguage());
		String participantInd = StringUtils.trim(fundCheckL2Form.getParticipantNoticeInd());
		...
		if(StringUtils.isNotBlank(fundCheckL2Form.getSelectedSeason()) 
				&& StringUtils.isNotBlank(fundCheckL2Form.getSelectedYear())) {
