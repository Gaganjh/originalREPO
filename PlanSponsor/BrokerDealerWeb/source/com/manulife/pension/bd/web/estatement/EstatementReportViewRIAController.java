 ...
 */
@Controller
@RequestMapping(value ="/ViewRIAStatements")


public class EstatementReportViewRIAController extends BaseReportController {
	...
	public EstatementReportViewRIAController() {
		super(EstatementReportViewRIAController.class);
	} 

	@RequestMapping(value ="/ViewRIAStatements",  method =  {RequestMethod.GET}) 
	...
	protected String doCommon(
		...
		//paging
		estatementReportForm.setRiaStatementListVO(paging(riaStatementListVO,
				estatementReportForm, request));
	 ...
	 * @throws SystemException
	 */
	@RequestMapping(value ="/ViewRIAStatements",params= {"task=sort"},  method =  {RequestMethod.GET}) 
	...
	throws  SystemException {
		...
		// Hard coding for development purpose
		estatementReportForm.setUserType(userType);
		if (profile.isInternalUser()) {
			// if (StringUtils.equals(userType,
			// BDUserRoleType.RIAUserManager.getUserRoleCode())){
			forward = forwards.get("internal");
		} else {

			// Need to write a query to fetch data from Organaisation Tables etc

			// populate a new vo for firms list

			/*
			 * List<Date> listDate = new ArrayList<Date>(); DateFormat
			 * dateFormatter = new SimpleDateFormat("MM-dd-yyyy");
			 * 
			 * 
			 * try { listDate.add(dateFormatter.parse("09-30-2015"));
			 * listDate.add(dateFormatter.parse("10-06-2015"));
			 * listDate.add(dateFormatter.parse("07-10-2015"));
			 * listDate.add(dateFormatter.parse("11-09-2015"));
			 * listDate.add(dateFormatter.parse("06-05-2015"));
			 * listDate.add(dateFormatter.parse("03-05-2015"));
			 * listDate.add(dateFormatter.parse("02-01-2015"));
			 * listDate.add(dateFormatter.parse("08-12-2015"));
			 * 
			 * } catch (java.text.ParseException e) { // TODO Auto-generated
...
//						if(listFirms != null && !listFirms.contains(riaStatementVO.getFirmName())){
//							listFirms.add(riaStatementVO.getFirmName());
//						}
					}
...
//			riaStatementsList = getRiaStatementList(estatementReportForm.getSelectedFirmId()); 
...
//			List<RiaStatementVO> riaStatementsList = new ArrayList<RiaStatementVO>();
//
//			riaStatementsList = getRiaStatementList(riaFrimId);
//
//			// Setting the value in the form
//			if (riaStatementsList != null) {
//				estatementReportForm.setRiaStatementListVO(riaStatementsList);
//			}
		...
		return forward;// /estatement/fetch
	}
	...
	private List<RiaStatementVO> getRiaStatementList(String selectedFirmId) {
		...
		// Testing Purpose need to be replace later by TIBCO Response 
		// Creating  List
		/*RiaStatementVO rvo= new RiaStatementVO();
		rvo.setCsvDocId("csvDocId");
		rvo.setPdfDocId("pdfDocId");
		rvo.setFirmName("RIA Firm");
		rvo.setGenDate(new Date());
		riaStatementsList.add(rvo);
		riaStatementsList.add(rvo);
		riaStatementsList.add(rvo);
		riaStatementsList.add(rvo);
		riaStatementsList.add(rvo);
		...
		riaStatementsList.add(rvo);
		riaStatementsList.add(rvo);
		System.out.println("entered the list" + riaStatementsList.size());*/
		return riaStatementsList;
	}
	 ...
	 * @throws SystemException
	 */
	@RequestMapping(value ="/ViewRIAStatements",params= {"task=downloadDocuments"},  method =  {RequestMethod.GET}) 
