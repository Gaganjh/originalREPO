...
/**
 * The last step of register firm rep. It allows the user to login from this
 * page.
 * 
 * @author guweigu
 * 
...
public class RegisterFirmRepStep3Controller extends BDPublicWizardProcessController {
	...
	static{
		forwards.put("input","/registration/registerFirmRepStep3.jsp");
	...
	private static Map<String, Integer> localSecurityServiceMap = new HashMap<String, Integer>(3);

	static {
		localSecurityServiceMap.put(DisabledUserException.class.getName(), BDErrorCodes.USER_PROFILE_DELETED);
	}

	public RegisterFirmRepStep3Controller() {
		super(RegisterFirmRepStep3Controller.class, RegisterFirmRepProcessContext.class,
				RegisterFirmRepProcessContext.ProcessName, RegisterFirmRepProcessContext.Step3);
	 ...
	 */
	@Override
	protected ProcessState doContinue( ActionForm form, HttpServletRequest request,
		...
		try {
			List<ValidationError> errors = LoginHandler.getInstance().doLogin(context.getUserName(),
					context.getPassword(), request, response, localSecurityServiceMap);
			// TODO:User selects Log Me In button and the user profile status is
			// deleted. Validation
			// pending.
			if (logger.isDebugEnabled()) {
