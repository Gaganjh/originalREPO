...
public class SDUHistoryTabController extends BDReportController {
	...
	static {
		forwards.put("default","/secureDocumentUpload/sduHistoryTab.jsp");
		forwards.put("sort","/secureDocumentUpload/sduHistoryTab.jsp");
		forwards.put("filter","/secureDocumentUpload/sduHistoryTab.jsp");
		forwards.put("page","/secureDocumentUpload/sduHistoryTab.jsp");
		forwards.put("print","/secureDocumentUpload/sduHistoryTab.jsp");
		forwards.put("input","/secureDocumentUpload/sduHistoryTab.jsp");
	}
	...
	private static final String DOWNLOAD_COLUMN_HEADING_SUBMISSIONS = "Submission Number,Submission Date/Time,Document Name,Submitted By,Role,Upload Status";
	...
	private static final String XSLT_FILE_KEY_NAME = "SDUHistoryTabReport.XSLFile";
	private static final String RECEIVED_DATE_MASK_PATTERN = "MM/dd/yyyy hh:mm a";
	...
	protected void populateReportCriteria(ReportCriteria reportCriteria, BaseReportForm form,
		...
		reportCriteria.addFilter(SDUHistoryTabReportData.FILTER_CONTRACT_NO, contractId);		
		...
		reportCriteria.addFilter(SDUHistoryTabReportData.FILTER_SDU_CLIENT_ID,SDUConstants.SDU_BD_CLIENT_ID);
	...
	protected Collection doValidate(ActionForm form, HttpServletRequest request) {
		...
		if (FILTER_TASK.equals(request.getParameter(TASK_KEY)) 
				|| PRINT_PDF_TASK.equals(request.getParameter(TASK_KEY)) 
			...
			try {
				...
				if ((!StringUtils.isEmpty(sduHistoryTabForm.getFilterStartDate())) && fromDate != null) {
					...
					if (calFromDate.before(calEarliestDate)) {
						errors.add(new GenericException(BDErrorCodes.FROM_DATE_BEFORE_24_MONTHS));
				...
				if (toDate != null && toDate.after(today)) {
					errors.add(new GenericException(BDErrorCodes.TO_DATE_AFTER_TODAY));
				}
		...
		if (!errors.isEmpty()) {
			...
			request.setAttribute(SDUConstants.REPORT_BEAN, emptyReport);
		}
		return errors;
	}
	...
	public String doCommon(BaseReportForm actionForm, HttpServletRequest request,
		...
		/*
		 * FunctionalLogger.INSTANCE.log("Submission Document History",
		 * getUserProfile(request), getClass(), getMethodName(actionForm, request));
		 */
	...
	public String doDefault(@Valid @ModelAttribute("sduHistoryTabForm") SDUHistoryTabForm actionForm,BindingResult bindingResult, HttpServletRequest request, HttpServletResponse response)
		...
		if (!StringUtils.equals(getTask(request), PRINT_PDF_TASK)) {
	...
	protected void populateReportForm(BaseReportForm reportForm, HttpServletRequest request) {
		...
		if ( StringUtils.equals(getTask(request), DEFAULT_TASK)) {
			...
			sduHistoryTabForm.setPendingContract(SDUHelper.isPreActiveContract(bob.getCurrentContract().getStatus()));
	...
	*/
	...
	private ReportData getSubmissionHistory(ReportCriteria reportCriteria,HttpServletRequest request) throws SystemException {
		...
		try {		
			...
			if ( StringUtils.equals(getTask(request), DOWNLOAD_TASK) || StringUtils.equals(getTask(request), PRINT_PDF_TASK)) {
				pagedResults=false;
			}
			
			JsonGenerator payload = nodeFactory.createGenerator(stream, JsonEncoding.UTF8);
			payload.writeStartObject();
			payload.writeStringField("clientId", SDUConstants.SDU_BD_CLIENT_ID);
	...
	protected byte[] getDownloadData(BaseReportForm reportForm, ReportData report, HttpServletRequest request)
		...
		try {
			...
			BobContext bob = BDSessionHelper.getBobContext(request) ;
			int contractId = bob.getCurrentContract().getContractNumber();
			buffer.append("Contract").append(COMMA).append(contractId).append(COMMA)
					.append(bob.getCurrentContract().getCompanyName());
    ...
    protected String getFileName(HttpServletRequest request) {
		...
		BobContext bob = BDSessionHelper.getBobContext(request) ;
		int contractId = bob.getCurrentContract().getContractNumber();	
		return "SubmissionHistory-" + String.valueOf(contractId)+ CSV_EXTENSION;
	}
	@RequestMapping(value = "/history/",params = {"task=printPDF"}, method = {RequestMethod.GET})
	public String doPrintPDF(@Valid @ModelAttribute("sduHistoryTabForm") SDUHistoryTabForm form,
			BindingResult bindingResult, HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	    ...
	    public Document prepareXMLFromReport(BaseReportForm reportForm,
		 ...
		 SDUHistoryTabForm form = (SDUHistoryTabForm) reportForm;
		 SDUHistoryTabReportData data = (SDUHistoryTabReportData) report;
	        int rowCount = 1;
	        int maxRowsinPDF;
	        ...
	        LayoutPage layoutPageBean = getLayoutPage(BDPdfConstants.SDU_HISTORY_PATH, request);

	        Element rootElement = doc.createRootElement(BDPdfConstants.SUBMIT_TXN_HISTORY);
	      ...
	      //  setRothMessageElement(doc, rootElement, request);
	        ...
	        String bodyHeader1 = BDPdfConstants.SUBMIT_TXN_HISTORY_HEADER;
	        String IntroTxt = StringUtils.EMPTY;
	        try {
				Miscellaneous content = (Miscellaneous) ContentCacheManager
						.getInstance().getContentById(BDContentConstants.SDU_HISTORY_TAB_INTRO,
	        ...
	        PdfHelper.convertIntoDOM(PdfConstants.INTRO2_TEXT, rootElement, doc, IntroTxt);
	        doc.appendTextNode(rootElement, BDPdfConstants.FROM_DATE,form.getFilterStartDate() 
	                );

	        doc.appendTextNode(rootElement, BDPdfConstants.TO_DATE, 
	        		form.getFilterEndDate());
	        ...
	        if (noOfRows > 0) {
	            ...
	            Iterator<SDUHistoryTabVO> iterator = data.getDetails().iterator();
	            ...
	            for (int i = 0; i < noOfRows && rowCount <= maxRowsinPDF; i++) {   
	                ...
	                SDUHistoryTabVO theItem = iterator.next();
	                // Sets main report.
	            ...
	            String	message = ContentHelper.getContentText(BDContentConstants.SDU_NO_RECORDS_TO_DISPLAY_MESSAGE, 
	 ...
	 private void setReportDetailsXMLElements(PDFDocument doc, Element txnDetailElement,
	    		SDUHistoryTabVO theItem) {
	        if (theItem != null) {
	        	doc.appendTextNode(txnDetailElement, BDPdfConstants.SUBMISSION_NO, Long.toString(theItem.getSubmissionId()));
	            doc.appendTextNode(txnDetailElement, BDPdfConstants.SUBMISSION_DATE, DateRender
	                    .formatByPattern(theItem.getSubmissionTs(), null,
	                    		RECEIVED_DATE_MASK_PATTERN));
	            doc.appendTextNode(txnDetailElement, BDPdfConstants.SUBMISSION_DOC_NAME, theItem.getFileName());
	            doc.appendTextNode(txnDetailElement, BDPdfConstants.SUBMITTED_BY, theItem.getClientUserName());
	            doc.appendTextNode(txnDetailElement, BDPdfConstants.SUBMISSION_ROLE, theItem.getClientUserRole());
	            doc.appendTextNode(txnDetailElement, BDPdfConstants.UPLOAD_STATUS, theItem.getSubmissionStatus());
	        }
	    }	
