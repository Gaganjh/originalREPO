...
@Component
public class RateOfReturnCalculatorDefinedBenifitValidator implements Validator {
	...
	private static Logger logger = Logger.getLogger(RateOfReturnCalculatorDefinedBenifitValidator.class);
	@Override
	...
	public void validate(Object target, Errors errors) {
        ...
        if (null == action || StringUtils.isBlank(action) || StringUtils.equals(DEFAULT, action) || StringUtils.equals(PRINT_PDF, action)
                || StringUtils.equals(BACK, action) || StringUtils.equals(RESET, action) || StringUtils.equals(CALCULATE, action)) {
            BDSessionHelper.removeErrorsInSession(request);
            if(StringUtils.equals(CALCULATE, action)) {
                    ...
                    if (!(null == rateOfReturnCalculatorForm.getTimePeriodFromToday())
                        ...
                        if (RateOfReturnCalculatorForm.VALID_TIME_PERIOD_FROM_TODAY_VALUES
                            ...
                            if (RateOfReturnCalculatorForm.TIME_PERIOD_FROM_DATE_RANGE
                                ...
                                // validate start date
                            	String startDateBreakUp[] = rateOfReturnCalculatorForm.getStartDate().split("/");
                            	
                            	if(startDateBreakUp.length == 3) {
                            		rateOfReturnCalculatorForm.setStartDay(startDateBreakUp[1]);
                            		rateOfReturnCalculatorForm.setStartMonth(startDateBreakUp[0]);
                            		rateOfReturnCalculatorForm.setStartYear(startDateBreakUp[2]);
                                    ...
                                    if ((result1 == DATE_EMPTY)) {
                                        error.add(new GenericException(
                                        		BDErrorCodes.ERROR_CALCADHOCROR_START_DATE_REQUIRED));
                                    } else {
                                        if (result1 == DATE_INVALID) {
                                            ...
                                            error.add(new GenericException(
                                            		BDErrorCodes.ERROR_CALCADHOCROR_START_DATE_1950));
                                        } else if (result1 == MONTH_INVALID) {
                                            error.add(new GenericException(
                                            		BDErrorCodes.ERROR_CALCADHOCROR_START_DATE_INVALID));
                                        } else if (result1 == DAY_INVALID) {
                                            error.add(new GenericException(
                                            		BDErrorCodes.ERROR_CALCADHOCROR_START_DATE_INVALID_1));
                                        }
                            	...
                            	String endDateBreakUp[] = rateOfReturnCalculatorForm.getEndDate().split("/");
                            	
                            	if(endDateBreakUp.length == 3) {
                            		rateOfReturnCalculatorForm.setEndDay(endDateBreakUp[1]);
                            		rateOfReturnCalculatorForm.setEndMonth(endDateBreakUp[0]);
                            		rateOfReturnCalculatorForm.setEndYear(endDateBreakUp[2]);
                                    ...
                                    if ((result2 == DATE_EMPTY)) {
                                        error.add(new GenericException(
                                        		BDErrorCodes.ERROR_CALCADHOCROR_END_DATE_REQUIRED));
                                    } else {
                                        if (result2 == DATE_INVALID) {
                                            ...
                                            error.add(new GenericException(
                                            		BDErrorCodes.ERROR_CALCADHOCROR_END_DATE_INVALID));
                                        } else if (result2 == DAY_INVALID) {
                                            error.add(new GenericException(
                                            		BDErrorCodes.ERROR_CALCADHOCROR_END_DATE_INVALID_1));
                                        }
                                ...
                                if (error.size() == 0) {
                                    ...
                                    // end date later then today
                                    if (calendarEndDate.compareTo(today)>=0) {

                                        error.add(new GenericException(
                                        		BDErrorCodes.ERROR_CALCADHOCROR_END_DATE_LATER_THEN_TODAY));
                                    }
                                    ...
                                    if (calendarStartDate.after(calendarEndDate)) {
                                        ...
                                        error.add(new GenericException(
                                        		BDErrorCodes.ERROR_CALCADHOCROR_START_DATE_LATER_THEN_END_DATE));
                                    }
                            ...
                            error.add(new GenericException(
                            		BDErrorCodes.ERROR_CALCADHOCROR_TIME_PERIOD_FROM_TODAY_INVALID));
                        }
                    } else {
                        // no radio button selected
                        error.add(new GenericException(
                        		BDErrorCodes.ERROR_CALCADHOCROR_TIME_PERIOD_NOT_SELECTED));
                    }

                    if (error.size() > 0) {
                        BDSessionHelper.setErrorsInSession(request, error);
