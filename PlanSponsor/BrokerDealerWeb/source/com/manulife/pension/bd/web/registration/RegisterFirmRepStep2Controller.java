...
import com.manulife.pension.service.security.bd.valueobject.FirmRepRegistrationValueObject;
...
/**
 * The step 2 of the register firm rep. Setup the address, user id, password, challenge, etc...
 * 
 * @author guweigu
 * 
 */
...
public class RegisterFirmRepStep2Controller extends BDPublicWizardProcessController {
	private String userName = null;
	@ModelAttribute("registerFirmRepStep2Form") 
	public RegisterFirmRepStep2Form populateForm() 
	{
		return new RegisterFirmRepStep2Form();
		}
	public static HashMap<String,String>forwards = new HashMap<String,String>();
	static{
		forwards.put("input","/registration/registerFirmRepStep2.jsp");
		forwards.put("fail","/registration/registerFirmRepStep2.jsp");
		}
    ...
    public RegisterFirmRepStep2Controller() {
        super(RegisterFirmRepStep2Controller.class, RegisterFirmRepProcessContext.class,
                RegisterFirmRepProcessContext.ProcessName, RegisterFirmRepProcessContext.Step2);
    }
    ...
    protected ProcessState doContinue( ActionForm form,
        ...
        // start changes for FRW Update Password Register Controller 
        RegisterFirmRepStep2Form registerFirmRepStep2Form = (RegisterFirmRepStep2Form) form;
        errors.addAll(validate((RegisterFirmRepStep2Form) form, request));
        
        // End changes for FRW Update Password Register Controller 
        String ipAddress = IPAddressUtils.getRemoteIpAddress(request);
        String username ;
		if (null != registerFirmRepStep2Form.getUserCredential()
				&& registerFirmRepStep2Form.getUserCredential().getUserId() != null) {
			username = registerFirmRepStep2Form.getUserCredential().getUserId();
		}else{
        	username = userName;
        }
        /*
		...
		PasswordMeterUtility.validateSeqConsCharForExUsers(username,
				registerFirmRepStep2Form.getUserCredential().getPassword(), errors);
        } else {
        ...
        if (errors.size() == 0) {
            ...
            FirmRepRegistrationValueObject regVO = createRegistrationVO(context.getCreationVO(),
                    (RegisterFirmRepStep2Form) form);
           ...
           // BDUserProfile userProfile = BDSessionHelper.getUserProfile(request);
            try {
                BDPublicSecurityServiceDelegate.getInstance().registerFirmRep(regVO, ipAddress, request.getSession(false).getId(), regVO.getUserProfileId());
                context.setUserName(regVO.getSecurityVO().getUserName());
                context.setPassword(regVO.getSecurityVO().getPassword());
            } catch (SecurityServiceException e) {
                logger.debug("Firm Rep Registration Failed. ", e);
    ...
    /**
     * Helper method to create the FirmRepRegistrationValueObject from the form object
     * 
     * @param form
     * @return FirmRepRegistrationValueObject
     */
    private FirmRepRegistrationValueObject createRegistrationVO(
            FirmRepCreationValueObject creationVO, RegisterFirmRepStep2Form form) {
        if (logger.isDebugEnabled()) {
            logger.debug("entry -> createRegistrationVO");
        }
        FirmRepRegistrationValueObject vo = new FirmRepRegistrationValueObject();
        vo.setFirstName(creationVO.getFirstName());
        vo.setLastName(creationVO.getLastName());
        vo.setEmailAddress(creationVO.getEmailAddress());
        vo.setPhoneNumber(creationVO.getPhoneNumber());
        ...
        siteVO.setDefaultSiteLocation(SiteLocation.valueOf(form.getDefaultSiteLocation()));
        siteVO.setProducerLicence(form.getProducerLicense());
        vo.setSiteInfoVO(siteVO);
        if (logger.isDebugEnabled()) {
     ...
     * @throws SystemException
     */
    private List<ValidationError> validate(RegisterFirmRepStep2Form form, HttpServletRequest request) throws SystemException {
        if (logger.isDebugEnabled()) {
            logger.debug("entry -> validate");
        }
        List<ValidationError> errors = new ArrayList<ValidationError>();
        AddressRule.getInstance().validate(RegisterFirmRepStep2Form.FIELD_ADDRESS, errors,
                form.getAddress());
        // User Name mandatory and standards must be met
        UserNameRule.getInstance().validate(RegisterFirmRepStep2Form.FIELD_USER_ID, errors,
                form.getUserCredential().getUserId());
        // Password mandatory and standards must be met
        pwdMandatoryRule.validate(RegisterFirmRepStep2Form.FIELD_PASSWORD, errors, form
                .getUserCredential().getPassword());

        // Confirm Password mandatory and standards must be met
        confirmedPwdMandatoryRule.validate(RegisterFirmRepStep2Form.FIELD_CONFIRMED_PASSWORD,
        ...
        if (StringUtils.isNotEmpty(form.getUserCredential().getPassword())
            ...
            Pair<String, String> passwordPair = new Pair<String, String>(form.getUserCredential()
            	...
            	NewPasswordRule.getInstance().validate(RegisterFirmRepStep2Form.FIELD_PASSWORD, errors,
            			passwordPair);
        }
        ...
        PasswordChallengeInputRule.getInstance().validate(RegisterFirmRepStep2Form.FIELD_CHALLENGE,
                errors, challengePair);
        licenseMandatoryRule.validate(RegisterFirmRepStep2Form.FIELD_PRODUCER_LICENSE, errors, form
                .getProducerLicense());
        fundListingMandatoryRule.validate(RegisterFirmRepStep2Form.FIELD_DEFAULT_SITE_LOCATION,
                errors, form.getDefaultSiteLocation());
        if (!form.getAcceptDisclaimer()) {
            errors.add(new ValidationError(RegisterFirmRepStep2Form.FIELD_ACCEPT_DISCLAIMER,
	...
	protected String doInput(@Valid @ModelAttribute("registerFirmRepStep2Form") RegisterFirmRepStep2Form form,
	...
	protected String doContinue(@Valid @ModelAttribute("registerFirmRepStep2Form") RegisterFirmRepStep2Form form,
			BindingResult bindingResult, HttpServletRequest request, HttpServletResponse response)
			throws SystemException, IOException, ServletException {
		if (logger.isDebugEnabled()) {
			logger.debug("entry -> doInput");
	...
	public String doCancel(@Valid @ModelAttribute("registerFirmRepStep2Form") RegisterFirmRepStep2Form form,
			BindingResult bindingResult, HttpServletRequest request, HttpServletResponse response)
			throws SystemException, IOException, ServletException {
		if (logger.isDebugEnabled()) {
			logger.debug("entry -> doInput");
	...
	@ResponseBody
	public String doDeApiAjaxCall(@Valid @ModelAttribute("registerFirmRepStep2Form") RegisterFirmRepStep2Form form, BindingResult bindingResult,
