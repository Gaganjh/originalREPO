 ...
 *  	- removed instance variable,"formattedDate" to avoid race condition issues(CP: 102715&102716)
 */
@Controller
@RequestMapping(value ="/tools")
@SessionAttributes({"viewContributionDetailsForm"})

public class ViewContributionDetailsController extends ReportController {
	...
	public static final String VIEW_CONTRIBUTION_DETAILS = "/tools/viewContributionDetails.jsp";
	static{
		forwards.put("input",VIEW_CONTRIBUTION_DETAILS);
		forwards.put("tools","redirect:/do/tools/toolsMenu/"); 
		forwards.put("default",VIEW_CONTRIBUTION_DETAILS); 
		forwards.put("sort",VIEW_CONTRIBUTION_DETAILS);
		forwards.put("filter",VIEW_CONTRIBUTION_DETAILS);
		forwards.put("page",VIEW_CONTRIBUTION_DETAILS);
		forwards.put("print",VIEW_CONTRIBUTION_DETAILS);
	 ...
	 * Constructor.
	 */
	public ViewContributionDetailsController() {
		super(ViewContributionDetailsController.class);
	}
	...
	private boolean isInEditableStatus(String status) {
		/*
		 * The following statuses indicate editable submissions.
		 */
		return FATAL_ERROR_STATUS.equals(status)
				|| UPLOAD_TO_APOLLO_FAILED_STATUS.equals(status)
				|| REJECT_WITH_ERRORS_STATUS.equals(status)
				|| SUSPEND_WITH_WARNINGS_STATUS.equals(status)
				|| DRAFT_STATUS.equals(status);
	}
	...
	throws SystemException {
        ...
        FunctionalLogger.INSTANCE.log("View Submission - Contribution", userProfile, subNo, getClass(), getMethodName( form, request));
		...
		if (isInEditableStatus(theItem.getSystemStatus()) &&
				statementInfoVO.isStatementsOutstanding()) {
	...
	@RequestMapping(value ="/viewContribution/", method =  {RequestMethod.GET}) 
	...
	@RequestMapping(value ="/viewContribution/",params= {"task=filter"}, method =  {RequestMethod.POST}) 
	public String doFilter (@Valid @ModelAttribute("viewContributionDetailsForm") ViewContributionDetailsForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws SystemException {
		String forward = null;
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	...
	@RequestMapping(value ="/viewContribution/",params= {"task=page"},method =  {RequestMethod.GET}) 
	...
	@RequestMapping(value ="/viewContribution/", params= {"task=sort"}, method =  {RequestMethod.GET}) 
	...
	@RequestMapping(value ="/viewContribution/" ,params= {"task=download"}, method =  {RequestMethod.GET}) 
	...
	@RequestMapping(value ="/viewContribution/" ,params= {"task=downloadAll"}, method =  {RequestMethod.GET}) 
	public String doDownloadAll (@Valid @ModelAttribute("viewContributionDetailsForm") ViewContributionDetailsForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws SystemException {
		String forward = null;
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	...
	protected byte[] getDownloadData(BaseReportForm reportForm, ReportData report,
		...
		UserProfile userProfile = getUserProfile(request);
		boolean isDefinedBenefit = userProfile.getCurrentContract().isDefinedBenefitContract();
		...
		while (columnLabels.hasNext()) {
			String columnLabel = columnLabels.next().toString();
			if (isDefinedBenefit) { // messy to setup at source setting, so we alter it here.
				if ("SSN#".equals(columnLabel)) {
					columnLabel = null; //skip
				} else if ("Participant Name".equals(columnLabel)) {
					columnLabel = "Name";
				} 
			} 
