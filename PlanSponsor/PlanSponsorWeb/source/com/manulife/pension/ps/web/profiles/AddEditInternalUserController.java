...
public class AddEditInternalUserController extends AbstractAddEditUserController {
	...
	static {
		forwards.put("input", "/profiles/addInternalUser.jsp");
		forwards.put("confirm", "redirect:/do/profiles/addInternalUser/?action=confirm");
		forwards.put("refresh", "redirect:/do/profiles/addInternalUser/?action=refresh");
		forwards.put("confirmPage", "/profiles/addInternalUserConfirmation.jsp");
	 ...
	 * Constructor.
	 */
	public AddEditInternalUserController() {
		super(AddEditInternalUserController.class);
		securityExceptionFormFieldMap.put(DuplicateEmployeeNumberException.class,
				AddEditUserForm.FIELD_EMPLOYEE_NUMBER);
		securityExceptionFormFieldMap.put(BDRvpUserExistException.class, AddEditUserForm.FIELD_RVP);
	}
	...
	protected void populateUserInfo(HttpServletRequest request, UserInfo userInfo, AddEditUserForm form) {
		 ...
		 */
		userInfo.setRole(UserRoleFactory.getUserRole(form.getPlanSponsorSiteRole()));
		...
		if (!bdRoleCode.equals(AccessLevelHelper.NO_ACCESS)) {
			role = BDUserRoleFactory.getInterUserRole(bdRoleCode);
			// if this is RVP, needs to set the rvp entity as well
			if (role != null && role.getRoleType().compareTo(BDUserRoleType.RVP) == 0) {
	...
	protected void populateForm(HttpServletRequest request, AddEditUserForm form, UserInfo userInfo) {
		...
		if (userInfo.getParticipantRole() != null) {
			form.setParticipantSiteRole(userInfo.getParticipantRole().toString());
		}
		if (userInfo.getRole() != null) {
			...
			form.setAccess408DisclosureRegen(
					userInfo.getRole().hasPermission(PermissionType.REGEN_408_DISCLOSURE) ? "Yes" : "No");
			form.setAccessIPIHypotheticalTool(
					userInfo.getRole().hasPermission(PermissionType.IPI_HYPOTHETICAL_TOOL) ? "Yes" : "No");
		}
		...
		form.setBdLicenceVerified(ManageInternalUserHelper.getLicenseVerifiedDisplay(userInfo.getBdLicenseVerified()));
	}
	...
	public String doDefault(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm actionForm,
	...
	protected UserInfo getUserInfo(Principal p, String userName) throws SystemException, SecurityServiceException {
		return SecurityServiceDelegate.getInstance().searchInternalUserByUserName(p, userName);
	}
	 ...
	 */
	protected void populateAccessLevelDropdown(HttpServletRequest request) throws SystemException {

		UserRole userRole = getUserProfile(request).getRole();
		// plan sponsor site drop down list
		List accessDropDownList = AccessLevelHelper.getInstance().getInternalAccessLevels(userRole);
		if (accessDropDownList != null) {
			request.setAttribute(Constants.INTERNAL_ACCESS_LEVEL, accessDropDownList);
		}
		// ezk site drop down list
		List ezkAccessDropDownList = AccessLevelHelper.getInstance().getEzkAccessLevels(userRole);
		if (ezkAccessDropDownList != null) {
			request.setAttribute(Constants.EZK_ACCESS_LEVEL, ezkAccessDropDownList);
		}

		// broker dealer site drop down list
		List bdAccessDropDownList = AccessLevelHelper.getInstance().getBDAccessLevels(userRole);
		if (bdAccessDropDownList != null) {
			request.setAttribute(Constants.BD_ACCESS_LEVEL, bdAccessDropDownList);
		}
	}
	 ...
	 * 
	 * @see
	 * com.manulife.pension.platform.web.controller.BaseAction#getSubmitAction(
	 * javax.servlet.http.HttpServletRequest, org.apache.struts.action.Form)
	 */
	@Override
	...
	@RequestMapping(value = "/addInternalUser/", params = { "action=cancel" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	public String doCancel(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,
			ModelMap model, HttpServletRequest request, HttpServletResponse response) throws SystemException {
	...
	@RequestMapping(value = "/addInternalUser/", params = { "action=save" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	...
	@RequestMapping(value = "/addInternalUser/", params = { "action=confirm" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	...
	@RequestMapping(value = "/addInternalUser/", params = { "action=refresh" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	...
	@RequestMapping(value = "/addInternalUser/", params = { "action=finish" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	public String doFinish(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,
			ModelMap model, HttpServletRequest request, HttpServletResponse response)throws IOException,ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
			if (errDirect != null) {
				request.getSession().removeAttribute(CommonConstants.ERROR_RDRCT);
	...
	@RequestMapping(value = "/addInternalUser/", params = { "action=printPDF" }, method = { RequestMethod.POST,
			RequestMethod.GET })
