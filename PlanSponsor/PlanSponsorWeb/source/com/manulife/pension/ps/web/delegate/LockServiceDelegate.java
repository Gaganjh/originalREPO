...
public class LockServiceDelegate extends PsAbstractServiceDelegate {
    ...
    public boolean lock(String componentName, String componentKey, long profileId,int totalNoOfAllowedLocks, boolean checkDuplicateUserLock, long lockExpiryTime) throws SystemException {
        boolean locked = false;
        LockService lockService = (LockService)getService();
        
        try {
            locked = lockService.lock(componentName, componentKey, profileId,totalNoOfAllowedLocks, checkDuplicateUserLock, lockExpiryTime);
        } catch (RemoteException e) {
        	logAndCreateSystemException(e);
        }
