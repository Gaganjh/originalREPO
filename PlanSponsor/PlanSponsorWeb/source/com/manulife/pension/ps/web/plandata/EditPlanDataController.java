 ...
 */
public class EditPlanDataController extends PsAutoController {
    ...
    public final String doDefault(
    	...
    	/*	final boolean lockObtained = checkIfUserObtainedLockOnContract(Integer.toString(contractId), request);
          ...
          //Notice Generation service - To load the CSF page with selected values
            ...
            return findForward( ACTION_FORWARD_DEFAULT);
    	}
		catch(Exception e){
    ...
    public String doInvestmentInformationSave(
        ...
        else
        {
            ...
            if(saveSuccess)
            {
                ...
                logInteraction(userProfile, EditPlanDataController.class.getName()+":doInvestmentInformationSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
            }
             }else
             {
            	 alerts.add(new ValidationError("saveInvInfoDataSuccess"
    		...
    		{ 
    			...
    			if(!userProfile.isInternalUser()) {
    				...
    				if(saveSuccess)
    				{
    					...
    					request.getSession().setAttribute(Constants.NOTICE_PLAN_DATA_VO,noticePlanVO);
    					logInteraction(userProfile, EditPlanDataController.class.getName()+":doContributionAndDistributionSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
    				}
    			}else
    			{
     ...
     * 
     */
    public String doSafeHarborSave( AutoForm actionForm, HttpServletRequest request,
            HttpServletResponse response) throws IOException, ServletException,
            SystemException {
        ...
        {   
            ...
            if(!userProfile.isInternalUser()) {
            	...
            	if(saveSuccess)
            	{
            		...
            		logInteraction(userProfile, EditPlanDataController.class.getName()+":doSafeHarborSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
            	}
            }else
    		{
    ...
    public String doAutomaticContributionSave(
       ...
       // validate form data editable fields
        ...
        if (null!=displayErrors&&displayErrors.size() > 0) {
        	...
        	if(userProfile.isInternalUser()) {
        		List<NoticePlanVestingMTExcludeVO> updatedExcludedMoneyTypes =getExcludedVesting(request, actionForm, contractNumber);
        ...
        form = noticePlanDataController.setValuesToForm(noticePlanVO, noticePlanCommonVO, form, form.getSelectedTab());
        request.getSession().setAttribute(Constants.NOTICE_PLAN_DATA_VO,noticePlanVO);
        logInteraction(userProfile, EditPlanDataController.class.getName()+":doAutomaticContributionSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
        }
            }else
            {
