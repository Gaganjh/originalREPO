...
public class EditTpaUserController extends AbstractTpaUserController {
	...
	static {
		forwards.put("input", "/profiles/editTpaUser.jsp");
		forwards.put("confirm", "redirect:/do/profiles/editTpaUser/?action=confirm");
		forwards.put("refresh", "redirect:/do/profiles/editTpaUser/?action=refresh");
		forwards.put("confirmPage", "/profiles/editTpaUserConfirmation.jsp");
		forwards.put("editUser", "/profiles/editTpaUser.jsp");
		forwards.put("manageUsers", "redirect:/do/profiles/manageTpaUsers/");
		forwards.put("new", "redirect:/do/profiles/manageTpaUsers/");
		forwards.put("changePermissions", "redirect:/do/profiles/tpaFirmUserPermissions/?action=edit&source=edit");
	 ...
	 * Constructor.
	 */
	public EditTpaUserController() {
		super(EditTpaUserController.class);
	...
	@RequestMapping(value = "/editTpaUser/", method = { RequestMethod.GET })
	...
	protected void populateUserInfo(HttpServletRequest request, UserInfo userInfo, AddEditUserForm form) {
		...
		if (form.getProfileStatus() == null && !form.isWebAccess()) {
			// For a New profile : When web access = No, permission's should be
			// set to default
			for (TpaFirm tpaFirm : form.getTpaFirms()) {
				...
				TPAActionHelper.copyAttributesToForm(tpaFirm.getContractAccess(0).getUserPermissions(), tpaFirm.getId(),
						form);
			}
		}
		...
		for (Iterator it = form.getTpaFirms().iterator(); it.hasNext();) {
			...
			if (tpaFirmForm.isRemoved()) {
				// need to send ContractPermissions with UserRole = null in this
				// case.
				ContractPermission permission = new ContractPermission(null); // role
																				// =
																				// NoAccess
				...
				if (ait.hasNext()) {
					...
					// want the confirmation page to have the correct values,
					// so...
					...
					if (!access.getPlanSponsorSiteRole().equals(AccessLevelHelper.NO_ACCESS)) {
						permission.setRole(UserRoleFactory.getUserRole(access.getPlanSponsorSiteRole()));
		...
		// To update special attributes for current contract in single TPA firm
		// view
	...
	private TpaFirm getTpaFirm(Integer tpaFirmId) throws SystemException {
		TPAFirmInfo info = TPAServiceDelegate.getInstance().getFirmInfo(tpaFirmId.intValue());
		if (info == null) {
			return null;
		}
	...
	@Autowired
	private AddEditTpaUserValidator addEditTpaUserValidator;

	@Autowired
	private AddEditTpaUserSaveValidator addEditTpaUserSaveValidator;
	...
	public void initBinder(HttpServletRequest request, ServletRequestDataBinder binder) {
		...
		/*
		 * binder.addValidators(addEditTpaUserValidator);
		 * binder.addValidators(addEditTpaUserSaveValidator);
		 */
	}
	...
	protected String doWarningsValidation(AddEditUserForm form, HttpServletRequest request) {
		 ...
		 * 
		 * // TTP.5 Iterator it = form.getTpaFirms().iterator();
		 * while(it.hasNext()) { TpaFirm tpaFirmForm = (TpaFirm) it.next();
		 * 
		 * // if count = 1 and if this has been turned off, and prior to save I
		 * had it, then // generate the warning. if
		 * ((tpaFirmForm.getILoanEmialPermissionCount() == 1) &&
		 * (form.getEmailPreferences().getReceiveiLoads() == false)) {
		 * AddEditUserForm originalForm = (AddEditUserForm)form.getClonedForm();
		 * if (originalForm.getEmailPreferences().getReceiveiLoads()) { if
		 * (warningMessages.length() >0) warningMessages.append("\\n");
		 * warningMessages.append("Firm(s): " + tpaFirmForm.getId() +
		 * " Warning!  This is the last user at the listed firm(s) with Receive i:loans Email permission."
		 * ); // 53972 } } }
		 * 
		 * if (warningMessages.length()>0) { request.setAttribute("WARNINGS",
	 ...
	 */
	@RequestMapping(value = "/editTpaUser/", params = { "action=addTpaFirm" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	public String doAddTpaFirm(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form,
		...
		if (!foundExisting) {
			...
			if (tpaFirm != null) {
				...
				UserInfo loginUserInfo = SecurityServiceDelegate.getInstance().getUserInfo(userProfile.getPrincipal());
				TPAUserContractAccessActionHelper.filterFirmContractAccess(loginUserInfo, tpaFirm);
				...
				TPAActionHelper.copyAttributesToForm(tpaFirm.getContractAccess(0).getUserPermissions(), tpaFirmId,
						form);
			...
			if (invalidTpaFirmId) {
				...
				errors.add(
						new ValidationError(AddEditUserForm.FIELD_TPA_FIRM_ID_TO_ADD, ErrorCodes.TPA_FIRM_ID_INVALID));
				SessionHelper.setErrorsInSession(request, errors);
			}
		}

		evaluateEmailDisplayRules(form, request);
		form.setGenerateChangeTrackingMessage(true); // signal to gen warning on
														// cancel
	...
	@RequestMapping(value = "/editTpaUser/", params = { "action=reload" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	...
	@RequestMapping(value = "/editTpaUser/", params = { "action=save" }, method = { RequestMethod.POST })
	public String doSave(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,
			ModelMap model, HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	...
	@RequestMapping(value = "/editTpaUser/", params = { "action=cancel" }, method = { RequestMethod.POST })
	...
	@RequestMapping(value = "/editTpaUser/", params = { "action=removeTpaFirm" }, method = { RequestMethod.POST })
	public String doRemoveTpaFirm(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form,
			BindingResult bindingResult, HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
			...
			if (tpaFirmForm.getId().toString().equals(form.getRemoveTpaFirmId())) {
				if (tpaFirmForm.isPersisted()) {
					if (errors.isEmpty()) {
						...
						form.setGenerateChangeTrackingMessage(true); // signal
																		// to
																		// gen
																		// warning
																		// on
	 ...
	 * 
	 */
	@RequestMapping(value = "/editTpaUser/", params = "actionLabel=change Permissions", method = { RequestMethod.POST,
			RequestMethod.GET })
	public String doChangePermissions(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form,
			BindingResult bindingResult, HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
		 ...
		 */
		TPAActionHelper.syncSettingsToUserPermissions(form, Integer.parseInt(form.getTpaFirmId()), true);
		request.setAttribute(Constants.SHOW_TPA_PERMISSIONS_ONLY, "true");
		return forwards.get(CHANGE_PERMISSIONS);
	}

	@RequestMapping(value = "/editTpaUser/", params = { "action=refresh" }, method = { RequestMethod.POST,RequestMethod.GET})
	public String doRefresh(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,
			HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	...
	@RequestMapping(value = "/editTpaUser/", params = { "action=confirm" }, method = { RequestMethod.POST,
			RequestMethod.GET })
	...
	@RequestMapping(value = "/editTpaUser/", params = { "action=finish" }, method = { RequestMethod.GET })
	 ...
	 * @throws SystemException
	 */
	@RequestMapping(value = "/editTpaUser/", params = { "action=changeWebAccess" }, method = { RequestMethod.POST })
	public String doChangeWebAccess(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form,
		...
		request.getSession(false).setAttribute(FLOW_TOKEN, "changeWebAccess");
		model.addAttribute("mapping", "edit");
		// In Add action, when web access is set to No, all permissions will be
		// set to default
	...
	protected Collection doValidate(ActionForm actionForm, HttpServletRequest request) {
		...
		if (form.isSaveAction()) {
			...
			if (StringUtils.isNotEmpty(form.getPhone().getValue())) {
				if (StringUtils.isEmpty(form.getPhone().getAreaCode())
						|| StringUtils.isEmpty(form.getPhone().getPhonePrefix())
						|| StringUtils.isEmpty(form.getPhone().getPhoneSuffix())
						|| form.getPhone().getValueTPA().length() < 10) {
					errors.add(new ValidationError(AddEditUserForm.FIELD_PHONE_NUMBER, ErrorCodes.PHONE_NOT_COMPLETE));
				}
				if (StringUtils.isNotEmpty(form.getPhone().getAreaCode())
						&& StringUtils.isNotEmpty(form.getPhone().getPhonePrefix())) {
			...
			FaxRule.getInstance().validate(AddEditUserForm.FIELD_FAX_NUMBER, errors, form.getFax().getValue());
			if (StringUtils.isNotEmpty(form.getFax().getValue())) {
				if (StringUtils.isEmpty(form.getFax().getAreaCode())
						|| StringUtils.isEmpty(form.getFax().getFaxPrefix())
						|| StringUtils.isEmpty(form.getFax().getFaxSuffix())
						|| form.getFax().getValueTPA().length() < 10) {
					errors.add(new ValidationError(AddEditUserForm.FIELD_FAX_NUMBER, ErrorCodes.FAX_NOT_COMPLETE));
				}
				if (StringUtils.isNotEmpty(form.getFax().getAreaCode())
						&& StringUtils.isNotEmpty(form.getFax().getFaxPrefix())) {
			...
			if (form.getEmail() != null && (form.getEmail().toLowerCase().indexOf("@jhancock") != -1
					|| form.getEmail().toLowerCase().indexOf("@manulife") != -1)) {
			...
			if (form.getSecondaryEmail() != null && (form.getSecondaryEmail().toLowerCase().indexOf("@jhancock") != -1
					|| form.getSecondaryEmail().toLowerCase().indexOf("@manulife") != -1)) {
				...
				if (!specialFirmFound)
					errors.add(new ValidationError(AddEditUserForm.FIELD_SECONDARY_EMAIL,
							ErrorCodes.SECONDARY_EMAIL_INVALID));
			}
			...
			TpaFirmRule.getInstance().validate(AddEditUserForm.FIELD_TPA_FIRM_ID_TO_ADD, errors,
		...
		if (!errors.isEmpty()) {
            SessionHelper.setErrorsInSession(request, errors);
            request.getSession(false).setAttribute(FLOW_TOKEN, "editUser");
