...
public class SubmissionUploadController extends AbstractSubmitController {
	...
	public SubmissionUploadForm populateForm()
{
	...
	static{
		forwards.put(INPUT,"/tools/submissionUpload.jsp");
		forwards.put("tools","redirect:/do/tools/toolsMenu/");
		forwards.put("confirm","redirect:/do/tools/submissionUpload/?action=confirm");
		forwards.put("refresh","redirect:/do/tools/submissionUpload/?action=refresh");
		forwards.put("confirmPage","/tools/submissionUploadConfirmation.jsp");
	...
	public SubmissionUploadController() {
		super(SubmissionUploadController.class);
	}

	
	@RequestMapping(value ="/submissionUpload/",  method =  {RequestMethod.GET}) 
	...
	throws SystemException {
		...
		try {
			...
			SessionHelper.setErrorsInSession(request, errors);
			return forwards.get(SUBMISSION_HISTORY);
		}
	 ...
	 */
	@RequestMapping(value ="/submissionUpload/" ,params={"actionLabel=send"}   , method =  {RequestMethod.POST}) 
	...
	protected void populateForm(SubmissionUploadForm form, UserProfile userProfile)
			throws SystemException {
		...
		form.setDisplayFileUploadSection(
		...
		form.setDisplayPaymentInstructionSection(!contract.getStatus().equals(Contract.STATUS_CONTRACT_DISCONTINUED)
                && !userProfile.isBeforeCAStatusAccessOnly()
		...
		if (marketClose != null && marketClose.after(currentDate)) {
			...
			if (msDiff < PsProperties.getNYStockClosureTimeLimit() * 60 * 1000) {
				...
				form.setMarketClose("The NYSE will close at " + sdf.format(marketClose)
		...
		try {
			...
			form.setAllowedMarketDates(dates);
		} catch (Exception e) {
			SystemException se = new SystemException(e, this.getClass().getName(),
		...
		form.setEmail(SecurityServiceDelegate.getInstance().getUserInfo(
	...
	@RequestMapping(value ="/submissionUpload/", params={"action=confirm"}, method =  {RequestMethod.GET}) 
	...
	@RequestMapping(value ="/submissionUpload/",params={"action=refresh"}, method =  {RequestMethod.GET}) 
