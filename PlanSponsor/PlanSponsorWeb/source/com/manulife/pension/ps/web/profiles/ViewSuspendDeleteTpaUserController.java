...
public class ViewSuspendDeleteTpaUserController extends AbstractTpaUserController {
	...
	static{
		forwards.put("input","/profiles/viewTpaUser.jsp");
		forwards.put("confirm","redirect:/do/profiles/editTpaUser/?action=confirm");
		forwards.put("refresh","redirect:/do/profiles/viewTpaUser/?action=refresh");
		...
		forwards.put("viewPermissions","redirect:/do/profiles/tpaFirmUserPermissions/?source=view&action=view"); 
		forwards.put("reset","redirect:/do/profiles/viewTpaUser/?action=refreshReset");
		forwards.put("unlock","redirect:/do/profiles/viewTpaUser/?action=refreshUnlock");
		forwards.put("resetPage","/password/resetPassword.jsp"); 
		forwards.put("unlockPage","/password/unlockPassword.jsp");
	...
	private static final String RESET_ACTION = "reset"; // confirmation processing
	private static final String RESET_PAGE = "resetPage"; // confirmation processing
	private static final String UNLOCK_PASSWORD_ACTION = "unlock"; // confirmation processing
	private static final String UNLOCK_PASSWORD_PAGE = "unlockPage"; // confirmation processing
	
	/**
	 * Constructor.
	 */
	public ViewSuspendDeleteTpaUserController() {
		super(ViewSuspendDeleteTpaUserController.class);
	}	
	...
	protected void populateUserInfo(HttpServletRequest request,
			UserInfo userInfo, AddEditUserForm actionForm) {
	...
	@RequestMapping(value = "/viewTpaUser/" ,params={"action=refresh"} , method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doRefresh(@Valid @ModelAttribute("addEditUserForm") AddEditUserForm actionForm, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
...
//		resetTpaDropdowns( form, request);
	 ...
	 */
	@RequestMapping(value ="/viewTpaUser/", method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doDefault (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form,BindingResult bindingResult,ModelMap model,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
	...
	return forwards.get(errDirect)!=null?forwards.get(errDirect):forwards.get("input");//if input forward not //available, provided default
		...
		if (TpaumHelper.isTPAUM(userProfile)) {
			...
			if (isAddUser(mapping) && tpaumFirms.size() == 1) {
				...
				TPAFirmInfo tpaFirm = (TPAFirmInfo) tpaumFirms.iterator()
						.next();
	...
	@RequestMapping(value ="/viewTpaUser/", params={"action=reset password"} , method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doResetPassword (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
	...
	return forwards.get(errDirect)!=null?forwards.get(errDirect):forwards.get("input");//if input forward not //available, provided default
        ...
        Collection<GenericException> errors1 = TPAUserContractAccessActionHelper.checkForLockOrDelete(userProfile, lockedUserProfileId, form.getUserName());
        if (!errors1.isEmpty()) {
            SessionHelper.setErrorsInSession(request, errors1);
	...
	String forward = (RESET_ACTION);
		try {
			...
			errors1.add(new GenericException(Integer.parseInt(e.getErrorCode())));
			SessionHelper.setErrorsInSession(request, errors1);
			return forwards.get(REFRESH_ACTION);
		}
		finally
    ...
    // code lifted from managePasswordAction.
	@RequestMapping(value ="/viewTpaUser/", params={"action=unlockPassword"}  , method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doUnlockPassword (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
	...
	@RequestMapping(value ="/viewTpaUser/", params={"action=suspend"}, method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doSuspend (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
	...
	@RequestMapping(value ="/viewTpaUser/" ,params={"action=unsuspend"}  , method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doUnsuspend (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
	...
	@RequestMapping(value ="/viewTpaUser/", params={"action=delete"} , method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doDelete (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
	...
	@RequestMapping(value ="/viewTpaUser/",params={"action=back"},  method =  {RequestMethod.POST,RequestMethod.GET}) 
	...
	@RequestMapping(value ="/viewTpaUser/",params="action=view permissions",  method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doViewPermissions (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
 ...
 /*
	...
	//// Note: In support of the view's reset/unlock, not sure why it's done like this but since it works
	//// in the managePasswordAction this way(and we want to re-use the confirmation pages) I will
	 ...
	 */
	@RequestMapping(value ="/viewTpaUser/",params={"action=refreshReset"},method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doRefreshReset (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form, BindingResult bindingResult,ModelMap model,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
...
//		}
	 ...
	 * @return The input forward.
	 */
	@RequestMapping(value ="/viewTpaUser/", params={"action=refreshUnlock"}, method =  {RequestMethod.POST,RequestMethod.GET}) 
	public String doRefreshUnlock (@Valid @ModelAttribute("addEditUserForm") AddEditUserForm form,BindingResult bindingResult,ModelMap model,HttpServletRequest request,HttpServletResponse response) 
	throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	       if(errDirect!=null){
...
*/
	...
	@RequestMapping(value ="/viewTpaUser/", params={"action=reload"} , method =  {RequestMethod.POST,RequestMethod.GET}) 
	...
	@RequestMapping(value ="/viewTpaUser/", params={"action=printPDF"}, method =  {RequestMethod.POST,RequestMethod.GET}) 
