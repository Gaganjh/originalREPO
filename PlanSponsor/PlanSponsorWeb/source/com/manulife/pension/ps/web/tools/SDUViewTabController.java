...
public class SDUViewTabController extends ReportController {
	...
	static {
		forwards.put("default","/tools/sduViewTab.jsp");
		forwards.put("sort","/tools/sduViewTab.jsp");
		forwards.put("page","/tools/sduViewTab.jsp");
		forwards.put("input","/tools/sduViewTab.jsp");
	...
	protected void populateReportCriteria(ReportCriteria reportCriteria, BaseReportForm form,
		...
		reportCriteria.addFilter(SDUViewTabReportData.REQ_SDU_CLIENT_ID,SDUConstants.SDU_PS_CLIENT_ID);
		// get the user profile object
		UserProfile userProfile = getUserProfile(request);
		Contract currentContract = userProfile.getCurrentContract();
		reportCriteria.addFilter(SDUViewTabReportData.REQ_CONTRACT_NO, currentContract.getContractNumber());		
	...
	protected void populateReportForm(BaseReportForm reportForm, HttpServletRequest request) {
		...
		//For external users, display only the files shared with them
		...
		//For allowed internal users, allow access to view the share history / share functionality
		else if (userProfile.getRole() instanceof CAR || userProfile.getRole() instanceof TeamLead
		...
		//For other internal users except above, block access to view the share history / share functionality
		else {
        	...
        	sduViewTabForm.setDownloadedByUserId(String.valueOf(userProfile.getPrincipal().getProfileId()));
        	sduViewTabForm.setDownloadedByUserName(userProfile.getPrincipal().getFirstName()+" "+userProfile.getPrincipal().getLastName());
            sduViewTabForm.setDownloadedByUserRole(userProfile.getPrincipal().getRole().getDisplayName());
	...
	*/
	...
	private ReportData getShareHistory(ReportCriteria reportCriteria,HttpServletRequest request) throws SystemException {
		...
		try {		
			...
			payload.writeStringField(SDUViewTabReportData.REQ_SDU_CLIENT_ID, SDUConstants.SDU_PS_CLIENT_ID);
