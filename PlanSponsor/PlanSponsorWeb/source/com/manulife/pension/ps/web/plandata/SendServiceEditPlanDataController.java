...
@SessionAttributes({"tabPlanDataForm"})
public class SendServiceEditPlanDataController extends PsAutoController {
	
	
	
	@ModelAttribute("tabPlanDataForm") 
     ...
     */
    private static final Logger logger = Logger.getLogger(SendServiceEditPlanDataController.class);
     ...
     * {@inheritDoc}
     */
	//DO get/post is both required
	@RequestMapping(value="/", method ={RequestMethod.POST,RequestMethod.GET})
    public final String doDefault(
    		@Valid @ModelAttribute("tabPlanDataForm") TabPlanDataForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
	...
	@RequestMapping(value="/",params= {"action=investmentInformationSave"}, method =  {RequestMethod.POST})
	public String doInvestmentInformationSave(
			@Valid @ModelAttribute("tabPlanDataForm") TabPlanDataForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
    			    throws IOException,ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
        ...
        else
        {
            ...
            if(saveSuccess)
            {
                ...
                logInteraction(userProfile, SendServiceEditPlanDataController.class.getName()+":doInvestmentInformationSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
            }
             }else
             {
            	 alerts.add(new ValidationError("saveInvInfoDataSuccess"
     ...
     */
	@RequestMapping(value="/", params= {"action=contributionAndDistributionSave"}, method =  {RequestMethod.POST})
    public String doContributionAndDistributionSave(
    		@Valid @ModelAttribute("tabPlanDataForm") TabPlanDataForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
    			    throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
    		...
    		{ 
    			...
    			if(!userProfile.isInternalUser()) {
    				...
    				if(saveSuccess)
    				{
    					...
    					request.getSession().setAttribute(Constants.NOTICE_PLAN_DATA_VO,noticePlanVO);
    					logInteraction(userProfile, SendServiceEditPlanDataController.class.getName()+":doContributionAndDistributionSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
    				}
    			}else
    			{
     ...
     * 
     */
	@RequestMapping(value="/",params= {"action=safeHarborSave"}, method =  {RequestMethod.POST})
    public String doSafeHarborSave( @Valid @ModelAttribute("tabPlanDataForm") TabPlanDataForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
		    throws IOException,ServletException, SystemException {
if(bindingResult.hasErrors()){
    String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
   if(errDirect!=null){
...
return forwards.get(errDirect)!=null?forwards.get(errDirect):forwards.get("error");//if input forward not //available, provided default
        ...
        {   
            ...
            if(!userProfile.isInternalUser()) {
            	...
            	if(saveSuccess)
            	{
            		...
            		logInteraction(userProfile, SendServiceEditPlanDataController.class.getName()+":doSafeHarborSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
            	}
            }else
    		{
 ...
 /*   
    ...
    protected Collection doValidate( Form form,
    	...
    	penErrors =	PsValidator1.doValidatePenTestAutoAction(form,  request, "error");
        return penErrors;
    } */
     ...
     * @throws SystemException
     */
    @RequestMapping(value="/",params= {"action=automaticContributionSave"}, method =  {RequestMethod.POST})
    public String doAutomaticContributionSave(
    		@Valid @ModelAttribute("tabPlanDataForm") TabPlanDataForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
    			    throws IOException,ServletException, SystemException {
		if(bindingResult.hasErrors()){
	        String errDirect =(String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
	...
	return forwards.get(errDirect)!=null?forwards.get(errDirect):forwards.get("error");//if input forward not //available, provided default
       ...
       // validate form data editable fields
        ...
        form = noticePlanDataController.setValuesToForm(noticePlanVO, noticePlanCommonVO, form, form.getSelectedTab());
        request.getSession().setAttribute(Constants.NOTICE_PLAN_DATA_VO,noticePlanVO);
        logInteraction(userProfile, SendServiceEditPlanDataController.class.getName()+":doAutomaticContributionSave", "Contract: "+form.getContractId().toString()+" Selected Tab: "+form.getSelectedTab()+" Data: "+printHistoryDataVO(historyData));
        }
            }else
            {
     ...
     * 
     */
    @RequestMapping(value="/",params= {"action=refresh"}, method =  {RequestMethod.POST})
    ...
    public final String doPrintPDF(
    		@Valid @ModelAttribute("tabPlanDataForm") TabPlanDataForm form, BindingResult bindingResult,HttpServletRequest request,HttpServletResponse response) 
    			    throws IOException,ServletException, SystemException {
		if (bindingResult.hasErrors()) {
			String errDirect = (String) request.getSession().getAttribute(CommonConstants.ERROR_RDRCT);
