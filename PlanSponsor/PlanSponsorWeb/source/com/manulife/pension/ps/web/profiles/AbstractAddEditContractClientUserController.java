...
public abstract class AbstractAddEditContractClientUserController extends PsAutoController {
	 ...
	 * @param clazz
	 */
	public AbstractAddEditContractClientUserController(Class clazz) {
		super(clazz);
	 ...
	 * 
	 * @param userInfo
	 *            The user info object to populate.
	 * @param actionForm
	 *            The action form to obtain values from.
	 * @return
	 */
	 ...
	 * 
	 * @param actionForm
	 *            The action form to populate.
	 * @param userInfo
	 *            The user info object to obtain values from.
	 */
	protected abstract void populateForm(HttpServletRequest request, AddEditClientUserForm actionForm,
	...
	public static Principal newPrincipal(Long profileId, String userName, UserRole role) {
		...
		try {
			return (Principal) theConstructor.newInstance(new Object[] { profileId, role, userName, null, null });
		} catch (Exception e) {
			return null;
		}
	}
	...
	/**
	 * Populates the common attributes of a user info. These includes user name,
	...
	private void populateCommonUserInfo(UserInfo userInfo, AutoForm actionForm) {
		...
		userInfo.setContactCommentText(form.getComments()); // TODO - to be
															// removed once
															// property is
															// removed from the
															// UserInfo class
	 ...
	 *            The action form to populate.
	 * @param userInfo
	 *            The user info object to obtain data from.
	 */
	private void populateCommonForm(AddEditClientUserForm form, UserInfo userInfo) {
		...
		form.setWebAccess(userInfo.isWebAccessInd() ? AddEditClientUserForm.FORM_YES_CONSTANT
				: AddEditClientUserForm.FORM_NO_CONSTANT);
	 ...
	 */
	protected String validate( ActionForm actionForm,ModelMap model,HttpServletRequest request) {

		AddEditClientUserForm form = (AddEditClientUserForm) actionForm;
		String mapping=(String)model.get("mapping");
		/*
		 * If we're not adding a user and the username is empty, we should
		 * forward back to the Manage Users page. Notice that this is possible
		 * when user hit back button on the confirmation page.
		 */
	...
	/**
	 * Perform validation that is common to all add/edit user actions.
	 * Specifically, this method validates first name, last name, email address
	 * and web access field
	 * 
	...
	protected Collection doValidate( ActionForm actionForm,ModelMap model, HttpServletRequest request) {
		...
		if (form.isSaveAction()) {
			NameRule.getFirstNameInstance().validate(AddEditClientUserForm.FIELD_FIRST_NAME, errors,
					form.getFirstName());
			NameRule.getLastNameInstance().validate(AddEditClientUserForm.FIELD_LAST_NAME, errors,
					form.getLastName());

			// check email & ssn base on web access field
			if (AddEditClientUserForm.FORM_NO_CONSTANT.equals(form.getWebAccess())) {
				// The following rules will fail if the email or ssn is empty so
				// we need to check that here
				if (form.getEmail() != null && form.getEmail().length() > 0) {
					EmailWithoutMandatoryRule.getInstance().validate(AddEditClientUserForm.FIELD_EMAIL, errors,
							form.getEmail());
				}
				if (form.getSsn().toString().length() > 0) {
					SsnWithoutMandatoryRule.getInstance().validate(AddEditClientUserForm.FIELD_SSN, errors,
							form.getSsn());
				}
			} else {
				// web access Yes need mandatory
				EmailRule.getInstance().validate(AddEditClientUserForm.FIELD_EMAIL, errors, form.getEmail());
				SsnRule.getInstance().validate(AddEditClientUserForm.FIELD_SSN, errors, form.getSsn());
			}
			
//			if (StringUtils.isNotEmpty(form.getMobileNumber().getValue())) {
//				MobileRule.getInstance().validate(AddEditClientUserForm.FIELD_MOBILE_NUMBER, errors,
//						form.getMobileNumber().getValue());
//				if (StringUtils.isEmpty(form.getMobileNumber().getAreaCode())
//						|| StringUtils.isEmpty(form.getMobileNumber().getPhonePrefix())
//						|| StringUtils.isEmpty(form.getMobileNumber().getPhoneSuffix())
//						|| form.getMobileNumber().getValue().length() < 10) {
//					errors.add(new ValidationError(AddEditClientUserForm.FIELD_MOBILE_NUMBER,
//							ErrorCodes.MOBILE_NOT_COMPLETE));
//				}
//				if (StringUtils.isNotEmpty(form.getMobileNumber().getAreaCode())
//						&& StringUtils.isNotEmpty(form.getMobileNumber().getPhonePrefix())) {
...
//						errors.add(new ValidationError(AddEditClientUserForm.FIELD_MOBILE_NUMBER,
//								ErrorCodes.MOBILE_INVALID));
//					}
//				}
//			}

			PhoneRule.getInstance().validate(AddEditClientUserForm.FIELD_TELEPHONE_NUMBER, errors,
					form.getTelephoneNumber().getValue());
			if (StringUtils.isNotEmpty(form.getTelephoneNumber().getValue())) {
				if (StringUtils.isEmpty(form.getTelephoneNumber().getAreaCode())
						|| StringUtils.isEmpty(form.getTelephoneNumber().getPhonePrefix())
						|| StringUtils.isEmpty(form.getTelephoneNumber().getPhoneSuffix())
						|| form.getTelephoneNumber().getValue().length() < 10) {
					errors.add(new ValidationError(AddEditClientUserForm.FIELD_TELEPHONE_NUMBER,
							ErrorCodes.PHONE_NOT_COMPLETE));
				}
				if (StringUtils.isNotEmpty(form.getTelephoneNumber().getAreaCode())
						&& StringUtils.isNotEmpty(form.getTelephoneNumber().getPhonePrefix())) {
					String areaCode = null, phonePrefix = null;
					areaCode = form.getTelephoneNumber().getAreaCode();
					phonePrefix = form.getTelephoneNumber().getPhonePrefix();
					if (areaCode.charAt(0) == '0' || areaCode.charAt(0) == '1' || phonePrefix.charAt(0) == '0'
							|| phonePrefix.charAt(0) == '1') {
						errors.add(new ValidationError(AddEditClientUserForm.FIELD_TELEPHONE_NUMBER,
			...
			FaxRule.getInstance().validate(AddEditClientUserForm.FIELD_FAX_NUMBER, errors,
					form.getFaxNumber().getValue());
			if (StringUtils.isNotEmpty(form.getFaxNumber().getValue())) {
				if (StringUtils.isEmpty(form.getFaxNumber().getAreaCode())
						|| StringUtils.isEmpty(form.getFaxNumber().getFaxPrefix())
						|| StringUtils.isEmpty(form.getFaxNumber().getFaxSuffix())
						|| form.getFaxNumber().getValue().length() < 10) {
					errors.add(new ValidationError(AddEditClientUserForm.FIELD_FAX_NUMBER,
							ErrorCodes.FAX_NOT_COMPLETE));
				}
				if (StringUtils.isNotEmpty(form.getFaxNumber().getAreaCode())
						&& StringUtils.isNotEmpty(form.getFaxNumber().getFaxPrefix())) {
					String areaCode = null, faxPrefix = null;
					areaCode = form.getFaxNumber().getAreaCode();
					faxPrefix = form.getFaxNumber().getFaxPrefix();
					if (areaCode.charAt(0) == '0' || areaCode.charAt(0) == '1' || faxPrefix.charAt(0) == '0'
							|| faxPrefix.charAt(0) == '1') {
						errors.add(new ValidationError(AddEditClientUserForm.FIELD_FAX_NUMBER,
								ErrorCodes.FAX_INVALID));
					}
				}
			}

			ExtensionRule.getInstance().validate(AddEditClientUserForm.FIELD_EXTENSION_NUMBER, errors,
					form.getTelephoneExtension());
			if (StringUtils.isEmpty(form.getTelephoneNumber().getValue())
					&& StringUtils.isNotEmpty(form.getTelephoneExtension())) {
				errors.add(new ValidationError(AddEditClientUserForm.FIELD_EXTENSION_NUMBER,
			...
			for (int i = 0; i < form.getContractAccesses().size(); i++) {
				...
				// scc86
				if (!emailContainsCompanyName && contractAccess.getContractNumber().intValue() != 70300
						&& contractAccess.getContractNumber().intValue() != 80089) {
				...
				// MPR38
				if (contractAccess.getPlanSponsorSiteRole().equals(null)
						|| contractAccess.getPlanSponsorSiteRole().equals("")) {
					errors.add(new ValidationError(
							"contractAccesses[" + i + "]." + ClientUserContractAccess.FIELD_PLANSPONSORSITE_ROLE,
							ErrorCodes.CHANGE_PERMISSION_WITHOUT_ROLE, params));
				} else {
					if (contractAccess.getPlanSponsorSiteRole().equals(IntermediaryContact.ID)
							&& (contractAccess.getRoleType() == null || "".equals(contractAccess.getRoleType()))) {
						errors.add(new ValidationError(
								"contractAccesses[" + i + "]." + ClientUserContractAccess.FIELD_ROLE_TYPE, 2537,
								params));
					}
				}
				...
				if (contractAccess.isNewContract()
						...
						&& !contractAccess.isCbcIndicator() && !contractAccess.isSelectedAccess()
				...
				Map<String, Boolean> lastUserFlagMap = ClientUserContractAccessActionHelper.getLastUserFlags(
						contractAccess.getContractNumber().intValue(), form.getProfileId(),
				...
				if (contractAccess.isCbcIndicator()) {
					if (isAddUser(mapping)) {
						...
						if (!contractAccess.isPrimaryContact()
								&& (contractAccess.getPrimaryContactProfileId() == form.getProfileId()
										|| contractAccess.getPrimaryContactProfileId() == 0)) {
							GenericException ex = new GenericException(1055, params);
							errors.add(ex);
						}
						...
						if (!contractAccess.isMailRecepient()
								&& (contractAccess.getMailRecipientProfileId() == form.getProfileId()
										|| contractAccess.getMailRecipientProfileId() == 0)) {
							GenericException ex = new GenericException(1056, params);
							errors.add(ex);
						}
						// SCC.193
						if (contractAccess.isTrusteeMailRecepient()
						...
						// SCC.194
						if (contractAccess.isSignatureReceivedAuthSigner()
						...
						// SCC.195
						if (contractAccess.isPrimaryContact()
								&& PayrollAdministrator.ID.equals(contractAccess.getPlanSponsorSiteRole().getValue())) {
							GenericException ex = new GenericException(1356, params);
							errors.add(ex);
						}
						// SCC.196
						if (contractAccess.isPrimaryContact()
								&& IntermediaryContact.ID.equals(contractAccess.getPlanSponsorSiteRole().getValue())) {
							GenericException ex = new GenericException(1357, params);
							errors.add(ex);
						}
						// SCC.197
						if (contractAccess.isSignatureReceivedTrustee()
								&& !AuthorizedSignor.ID.equals(contractAccess.getPlanSponsorSiteRole().getValue())
								&& !Trustee.ID.equals(contractAccess.getPlanSponsorSiteRole().getValue())
								&& !PlanSponsorUser.ID.equals(contractAccess.getPlanSponsorSiteRole().getValue())) {
	 ...
	 * @param response
	 */
	public String doCancel( AutoForm actionForm,ModelMap model, HttpServletRequest request,
	 ...
	 */
	public String doSave( AutoForm actionForm,ModelMap model, HttpServletRequest request,
			HttpServletResponse response) throws SystemException {
...
//				userInfo.setMobileNumber(form.getMobileNumber());
				UserInfo newUserInfo = service.addUser(principal, userInfo, Environment.getInstance().getSiteLocation(),
						checkDuplicateSSN);
				...
				if (form.isAddStaggingContact()) {
					...
					service.updateStagingContact(principal, userProfile.getCurrentContract().getContactId(),
							Environment.getInstance().getSiteLocation(), form.getStaggingContactId(),
							BigDecimal.valueOf(loggedInUserProfileId));
				}
			} else {
				if (logger.isDebugEnabled()) {
					logger.debug("updateUser");
				}
				service.updateUser(principal, userInfo, Environment.getInstance().getSiteLocation(), false,
						checkDuplicateSSN);
			...
			String duplicateSSNWarningMessage = generateDuplicateSSNWarningMessage(e.getSsnMatches(), ssn);
	...
	/**
	 * Determines if an application wide check for duplicate SSNs should be
	 * performed.
	 * 
	 * This check is only performed if the add/edit user request is initiated by
	 * an authorized internal user and the user has not already dismissed the
	 * warning.
	 * 
	 * @param principal
	 *            The logged in user performing an add/edit user request
	 * @param form
	 * @return Whether the add/edit user request should include a duplicate SSN
	 *         check
	 */
	private boolean getCheckDuplicateSSN(Principal principal, AddEditClientUserForm form) {
		...
		if (!form.isIgnoreSSNWarning()) {
			...
			if (role.isInternalUser() && role.hasPermission(PermissionType.MANAGE_EXTERNAL_USERS)
					&& !role.hasPermission(PermissionType.SELECTED_ACCESS) && form.getSsn() != null
					&& !form.getSsn().isEmpty()) {
	 ...
	 * 
	 * @param ssnMatches
	 *            The list of users with matching SSNs
	 * @return The duplicate SSN warning message to display
	 */
	private String generateDuplicateSSNWarningMessage(List<UserInfo> ssnMatches, String ssn) throws SystemException {

		StringBuffer warningMessage = new StringBuffer();

		try {
			Message message = (Message) ContentCacheManager.getInstance().getContentById(
					ContentConstants.WARNING_DUPLICATE_SSN_WITHIN_PSW, ContentTypeManager.instance().MESSAGE);
			...
			String[] params = { ssn, Integer.toString(numProfilesDisplayed), Integer.toString(totalProfiles) };
		...
		for (UserInfo user : ssnMatches) {
			...
			warningMessage.append("\\n");
			warningMessage.append(StringUtils.substring(contractNum, 0, MAX_CONTRACT_LENGTH_IN_SSN_WARNING));
			warningMessage.append(", ");
			warningMessage.append(StringUtils.substring(user.getLastName(), 0, MAX_LAST_NAME_LENGTH_IN_SSN_WARNING));
			warningMessage.append(" ");
			warningMessage.append(StringUtils.substring(user.getFirstName(), 0, MAX_FIRST_NAME_LENGTH_IN_SSN_WARNING));
			warningMessage.append(", ");
			warningMessage.append(
					StringUtils.substring(user.getRole().getDisplayName(), 0, MAX_ROLE_NAME_LENGTH_IN_SSN_WARNING));
		}
		return warningMessage.toString();
	}

	/**
	 * Method to remove any previous primary contact or mail recipient on the
	 * contracts
	 * 
	 * @param principal
	 * @param userInfo
	 ...
	 * @throws SystemException
	 */
	private void updateContractContacts(Principal principal, UserInfo userInfo)
			throws SecurityServiceException, SystemException {
		SecurityServiceDelegate service = SecurityServiceDelegate.getInstance();
		for (ContractPermission contractPermission : userInfo.getContractPermissions()) {
			if (contractPermission.isPrimaryContact() || contractPermission.isMailRecipient()
					|| contractPermission.isTrusteeMailRecepient() || contractPermission.isStatementIndicator()
					|| contractPermission.isIccDesignate() || contractPermission.isSendServiceDesignate()) {
				int contractNumber = contractPermission.getContractNumber();
				// Get the contract info since the cached contract info is stale
				Contract contract = ClientUserContractAccessActionHelper.getContract(contractPermission.getRole(),
						contractNumber);
				if (contract != null) {
					...
					// If this user has been designated as the primary contact
					// we need to change the existing primary contact to no
					if (contractPermission.isPrimaryContact() && contract.getPrimaryContact() != null) {
						...
						if (primaryContactProfileId != 0 && primaryContactProfileId != userInfo.getProfileId()) {
							primaryContactUserInfo = service.searchByProfileId(newPrincipal(new Long(0), null, null),
									primaryContactProfileId);
							primaryContactUserInfo.getContractPermission(contractNumber).setPrimaryContact(false);
						}
					}
					// If this user has been designated as the mail recipient we
					// need to change the existing mail recipient to no
					if (contractPermission.isMailRecipient() && contract.getMailRecipient() != null) {
						...
						if (mailRecipientProfileId != 0 && mailRecipientProfileId != userInfo.getProfileId()) {
							if (primaryContactUserInfo != null
									&& primaryContactUserInfo.getProfileId() == mailRecipientProfileId) {
								primaryContactUserInfo.getContractPermission(contractNumber).setMailRecipient(false);
							} else {
								mailRecipientUserInfo = service.searchByProfileId(newPrincipal(new Long(0), null, null),
										mailRecipientProfileId);
								mailRecipientUserInfo.getContractPermission(contractNumber).setMailRecipient(false);
							}
						}
					}

					// If this user has been designated as the trustee mail
					// recipient we need to change the existing trustee mail
					// recipient to no
					if (contractPermission.isTrusteeMailRecepient() && contract.getTrusteeMailRecipient() != null) {
						...
						if (trusteeMailRecipientProfileId != 0
								&& trusteeMailRecipientProfileId != userInfo.getProfileId()) {
							if (primaryContactUserInfo != null
									&& primaryContactUserInfo.getProfileId() == trusteeMailRecipientProfileId) {
								primaryContactUserInfo.getContractPermission(contractNumber)
										.setTrusteeMailRecepient(false);
							} else if (mailRecipientUserInfo != null
									&& mailRecipientUserInfo.getProfileId() == trusteeMailRecipientProfileId) {
								mailRecipientUserInfo.getContractPermission(contractNumber)
										.setTrusteeMailRecepient(false);
							} else {
								trusteeMailRecipientUserInfo = service.searchByProfileId(
										newPrincipal(new Long(0), null, null), trusteeMailRecipientProfileId);
								trusteeMailRecipientUserInfo.getContractPermission(contractNumber)
										.setTrusteeMailRecepient(false);
							}
						}
					}

					// If this user has been designated as the Participant
					// statement consultant we need to change the existing
					// participant statement consultant to no
					if (contractPermission.isStatementIndicator()
							&& contract.getParticipantStatementConsultant() != null) {
						long participantStatConsProfileId = contract.getParticipantStatementConsultant().getProfileId();
						if (participantStatConsProfileId != 0
								&& participantStatConsProfileId != userInfo.getProfileId()) {
							if (primaryContactUserInfo != null
									&& primaryContactUserInfo.getProfileId() == participantStatConsProfileId) {
								primaryContactUserInfo.getContractPermission(contractNumber)
										.setStatementIndicator(false);
							} else if (mailRecipientUserInfo != null
									&& mailRecipientUserInfo.getProfileId() == participantStatConsProfileId) {
								mailRecipientUserInfo.getContractPermission(contractNumber)
										.setStatementIndicator(false);
							} else if (trusteeMailRecipientUserInfo != null
									&& trusteeMailRecipientUserInfo.getProfileId() == participantStatConsProfileId) {
								trusteeMailRecipientUserInfo.getContractPermission(contractNumber)
										.setStatementIndicator(false);
							} else {
								participantStatConsultantUserInfo = service.searchByProfileId(
										newPrincipal(new Long(0), null, null), participantStatConsProfileId);
								participantStatConsultantUserInfo.getContractPermission(contractNumber)
										.setStatementIndicator(false);
							}
						}
					}
					...
					/*
					 * CL 126187 - User attributes assigned to more than one
					 * user
					 */

					// If this user has been designated as the Icc Designate we
					// need to change the existing Icc Designate to no
					if (contractPermission.isIccDesignate() && contract.getIccDesignate() != null) {
						...
						if (iccDesignateProfileId != 0 && iccDesignateProfileId != userInfo.getProfileId()) {
							if (primaryContactUserInfo != null
									&& primaryContactUserInfo.getProfileId() == iccDesignateProfileId) {
								primaryContactUserInfo.getContractPermission(contractNumber).setIccDesignate(false);
							} else if (mailRecipientUserInfo != null
									&& mailRecipientUserInfo.getProfileId() == iccDesignateProfileId) {
								mailRecipientUserInfo.getContractPermission(contractNumber).setIccDesignate(false);
							} else if (trusteeMailRecipientUserInfo != null
									&& trusteeMailRecipientUserInfo.getProfileId() == iccDesignateProfileId) {
								trusteeMailRecipientUserInfo.getContractPermission(contractNumber)
										.setIccDesignate(false);
							} else if (participantStatConsultantUserInfo != null
									&& participantStatConsultantUserInfo.getProfileId() == iccDesignateProfileId) {
								participantStatConsultantUserInfo.getContractPermission(contractNumber)
										.setIccDesignate(false);
							} else {
								iccDesignateUserInfo = service.searchByProfileId(newPrincipal(new Long(0), null, null),
										iccDesignateProfileId);
					...
					/*
					 * CR033 - User attributes assigned to more than one user. If this user has been designated as the SEND Service Designate we
					 * need to change the existing SEND Service Designate to no
					 */
					if (contractPermission.isSendServiceDesignate() && contract.getSendServiceDesignate() != null) {
						...
						if (sendServiceDesignateProfileId != 0 && sendServiceDesignateProfileId != userInfo.getProfileId()) {
							if (primaryContactUserInfo != null
									&& primaryContactUserInfo.getProfileId() == sendServiceDesignateProfileId) {
								primaryContactUserInfo.getContractPermission(contractNumber).setSendServiceDesignate(false);
							} else if (mailRecipientUserInfo != null
									&& mailRecipientUserInfo.getProfileId() == sendServiceDesignateProfileId) {
								mailRecipientUserInfo.getContractPermission(contractNumber).setSendServiceDesignate(false);
							} else if (trusteeMailRecipientUserInfo != null
									&& trusteeMailRecipientUserInfo.getProfileId() == sendServiceDesignateProfileId) {
								trusteeMailRecipientUserInfo.getContractPermission(contractNumber)
										.setSendServiceDesignate(false);
							} else if (participantStatConsultantUserInfo != null
									&& participantStatConsultantUserInfo.getProfileId() == sendServiceDesignateProfileId) {
								participantStatConsultantUserInfo.getContractPermission(contractNumber)
										.setSendServiceDesignate(false);
							} else {
								sendServiceDesignateUserInfo = service.searchByProfileId(newPrincipal(new Long(0), null, null),
										sendServiceDesignateProfileId);
								sendServiceDesignateUserInfo.getContractPermission(contractNumber).setSendServiceDesignate(false);
							}
						}
					}
					/* End of CR033 changes */

					if (primaryContactUserInfo != null) {
						service.updateUser(principal, Environment.getInstance().getSiteLocation(),
								primaryContactUserInfo);
					}
					if (mailRecipientUserInfo != null) {
						service.updateUser(principal, Environment.getInstance().getSiteLocation(),
								mailRecipientUserInfo);
					}
					if (trusteeMailRecipientUserInfo != null) {
						service.updateUser(principal, Environment.getInstance().getSiteLocation(),
								trusteeMailRecipientUserInfo);
					}
					if (participantStatConsultantUserInfo != null) {
						service.updateUser(principal, Environment.getInstance().getSiteLocation(),
								participantStatConsultantUserInfo);
					}
					if (iccDesignateUserInfo != null) {
						if (iccDesignateUserInfo.getProfileStatus()
								.equals(SecurityConstants.SUSPENDED_PROFILE_STATUS)) {
							service.updateUser(principal, iccDesignateUserInfo,
									Environment.getInstance().getSiteLocation(), true);
						} else {
							service.updateUser(principal, Environment.getInstance().getSiteLocation(),
									iccDesignateUserInfo);
						}
					}
	 ...
	 * @return
	 */
	public String doConfirm(AutoForm actionForm, HttpServletRequest request,
	 ...
	 */
	public String doRefresh( AutoForm actionForm,ModelMap model, HttpServletRequest request,
			HttpServletResponse response) throws SystemException {
		...
		String mapping=(String)model.get("mapping");
		UserInfo userInfo = ClientUserContractAccessActionHelper.getManagedUserInfo(userProfile.getPrincipal(),
				userName);
		if (userInfo != null && !isViewUser(mapping)) {
			Collection<GenericException> errors = ClientUserContractAccessActionHelper.checkForLockOrDelete(userProfile,
					userInfo.getProfileId(), userName);
			if (!errors.isEmpty()) {
				SessionHelper.setErrorsInSession(request, errors);
				// return mapping.findForward(MANAGE_USERS);
				//return mapping.findForward(Constants.FORWARD_PLANSPONSOR_CONTACTS);
		...
		if (isAddUser(mapping) && form.getAllContractAccesses().size() == 0) {
			...
			if (userProfile.getCurrentContract() != null) {
				...
				Contract loginContract = ClientUserContractAccessActionHelper.getContract(userProfile.getRole(),
						new Integer(contractNumber));

				if (loginUserInfo.getRole().isInternalUser()) {
					form.setFieldsEnableForInternalUser(loginUserInfo.getRole()
				...
				if (loginUserInfo.getRole().isExternalUser()) {
					...
					ClientUserContractAccessActionHelper.populateNewContractAccess(contractPermission.getRole(),
							accessForm, loginContract);
				} else {
					ClientUserContractAccessActionHelper.populateNewContractAccess(loginUserInfo.getRole(), accessForm,
							loginContract);
				}
				form.getAllContractAccesses().add(accessForm);

				// Since this is a new profile the logged in user can manage all
				// of this profile's contracts
				form.setCanManageAllContracts(true);

				/*
				 * Clone the form after the dummy contract access object is
				 * created in the add user case.
				 */
	 ...
	 * @return
	 */
	public String doFinish( AutoForm actionForm,ModelMap model, HttpServletRequest request,
			HttpServletResponse response) {
		clearSession(model,request);
		// return mapping.findForward(MANAGE_USERS);
		//return mapping.findForward(Constants.FORWARD_PLANSPONSOR_CONTACTS);
	 ...
	 */
	public String doDefault( AutoForm actionForm,ModelMap model,HttpServletRequest request,
			HttpServletResponse response) throws SystemException {
		...
		/*
		 * If we're on an edit page, populate the action form with the user info
		 * object.
		 */
		...
		if (!isAddUser(mapping)) {
			...
			if (!"confirm".equalsIgnoreCase(form.getClientUserAction())
					&& !"addContract".equalsIgnoreCase(form.getClientUserAction())) {
			...
			UserProfile userProfile = getUserProfile(request);
			if (!userProfile.isInternalServicesCAR()) {				
			...
			UserInfo userInfo = ClientUserContractAccessActionHelper.getManagedUserInfo(userProfile.getPrincipal(),
					userName);
			request.getSession(false).setAttribute("managedUserInfo", userInfo);
			...
			if (!isViewUser(mapping) && !"confirm".equalsIgnoreCase(form.getClientUserAction())) {
				Collection<GenericException> errors = ClientUserContractAccessActionHelper
						.checkForLockOrDelete(userProfile, profileId, userName);
				if (!errors.isEmpty()) {
					SessionHelper.setErrorsInSession(request, errors);
					// return mapping.findForward(MANAGE_USERS);
					//return mapping.findForward(Constants.FORWARD_PLANSPONSOR_CONTACTS);
			...
			/*
			 * If we're editing a user whose contract we don't have access to,
			 * we return to the Manage Users page. This is possible if someone
			 * removes all the contract access in the edit user page and hit the
			 * back button in the confirmation page.
			 */
	 ...
	 * session.
	 */
	public String doReset(AutoForm actionForm, HttpServletRequest request,ModelMap model,
		...
		AddEditClientUserForm form = (AddEditClientUserForm) actionForm;
		Collection<GenericException> errors = ClientUserContractAccessActionHelper.checkForLockOrDelete(userProfile,
				form.getProfileId(), form.getUserName());
		if (!errors.isEmpty()) {
			SessionHelper.setErrorsInSession(request, errors);
			// return mapping.findForward(MANAGE_USERS);
			//return mapping.findForward(Constants.FORWARD_PLANSPONSOR_CONTACTS);
	 ...
	 * to false.
	 */
	public String doUnlock(AutoForm actionForm,ModelMap model, HttpServletRequest request,
			HttpServletResponse response) throws IOException, ServletException, SystemException {
