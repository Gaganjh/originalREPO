...
<script language="javascript">
	...
	function disableLoans() {
		var disabled = true;
		
		var businessConverted = "${businessConverted}";
		var checkPayableToCode ="${checkPayableToCode}";
		...
		if(businessConverted == true && getRadioValue("allowOnlineLoans") == "No" ){
			disabled =  false;
			disableRadio("allowOnlineLoans", disabled);
		}
...
var withdrawals = "${csfForm.withdrawalInd}";
		if (withdrawals == "Yes" && checkPayableToCode == "TR" ) {
			disabled = true;
			disableRadio("participantWithdrawalInd", disabled);
		}
	}
...
var withdrawalInd = "${csfForm.withdrawalInd}";
	...
	function loansAlertMessage() {
		alert("<content:getAttribute beanName="onlineLoansYes" attribute="text" filter="true"/>");
	}
	function initPage() {
		if (!elementHasChanged("allowOnlineLoans") && getRadioValue("allowOnlineLoans") == "Yes" ) {
			disableRadio("allowOnlineLoans", true);
		}
		disableLoans();
...
<table border="0" cellpadding="0" cellspacing="0" width="698">
	<tbody>
        ...
        <tr class="datacell1">
			...
			<td colspan="2">&nbsp;
				<form:select path="planFrequency" id="planFrequency">
			...
			<ps:trackChanges name="csfForm" property="planFrequency" />
		</tr>
		...
		<%theIndex++; if (theIndex % 2 == 0) { %>
			...
			<td width="55">
<form:radiobutton path="autoPayrollInd" value="Yes" /> Yes</td>
<td width="247">
<form:radiobutton path="autoPayrollInd" value="No" /> No</td>
			<ps:trackChanges name="csfForm" property="autoPayrollInd" />
		</tr>
...
<tr class="<%=theIndex % 2 == 0?"datacell2":"datacell1"%>">
	...
	<td colspan="2">
		&nbsp;
		<form:select path="payrollFeedbackService" id="payrollFeedbackService">
...
<c:if test="${csfForm.isHideConsent ==false}">
				...
				<td width="302" colspan="2">
					...
					<tr>
						<td width="55">
<form:radiobutton path="consentInd" value="Yes" /> Yes</td>
						<td width="55">
<form:radiobutton path="consentInd" value="No" /> No</td>
						<td width="192">
<form:radiobutton path="consentInd" value="NA" /> N/A</td>
					</tr>
					</table>
				</td>
...
<c:if test="${csfForm.loanRecordKeepingInd == 'Yes'}">
				...
				<td width="55">
<form:radiobutton onclick="checkOnlineLoans('${businessConverted}','${checkPayableToCode}');" path="allowOnlineLoans" value="Yes"/> Yes
				</td>
				<td width="247">
<form:radiobutton onclick="checkOnlineLoans('${businessConverted}','${checkPayableToCode}');" path="allowOnlineLoans" value="No"/> No
				</td>
				<ps:trackChanges name="csfForm" property="allowOnlineLoans" />
			</tr>
				<% if (theIndex % 2 == 0) { %>
...
<c:if test="${csfForm.loansChecksMailedTo == 'TR'}">Trustee</c:if>
				...
				<ps:trackChanges name="csfForm" property="loansChecksMailedTo" />
			</tr>
			<% if (theIndex % 2 == 0) { %>
...
<c:if test="${csfForm.tpaFirmExists ==true}">
					<td width="55">
<form:radiobutton path="whoWillReviewLoanRequests" value="TPA" /> TPA
					</td>
					<td width="247">
<form:radiobutton path="whoWillReviewLoanRequests" value="PS" /> Plan Sponsor
					</td>
</c:if>
				<ps:trackChanges name="csfForm" property="whoWillReviewLoanRequests" />
			</tr>
			<% if (theIndex % 2 == 0) { %>
				...
				<td width="1" class="greyborder"><img src="/assets/unmanaged/images/spacer.gif" border="0" height="1" width="1"></td>
				<td width="55">
<form:radiobutton path="creatorMayApproveLoanRequestsInd" value="Yes" /> Yes
				</td>
				<td width="247">
<form:radiobutton path="creatorMayApproveLoanRequestsInd" value="No" /> No
				</td>
				<ps:trackChanges name="csfForm" property="creatorMayApproveLoanRequestsInd" />
			</tr>
			<% if (theIndex % 2 == 0) { %>
			...
			<% } %>
				...
				<td width="1" class="greyborder"><img src="/assets/unmanaged/images/spacer.gif" border="0" height="1" width="1"></td>
				<td width="55">
<form:radiobutton path="allowLoansPackageToGenerate" value="Yes" /> Yes</td>
				<td width="247">
<form:radiobutton path="allowLoansPackageToGenerate" value="No" /> No</td>
				<ps:trackChanges name="csfForm" property="allowLoansPackageToGenerate" />
			</tr>
...
<td width="302" colspan="2">&nbsp;&nbsp;${csfForm.withdrawalInd}</td>
			...
			<% } %>
				...
				<td width="1" class="greyborder"><img src="/assets/unmanaged/images/spacer.gif" border="0" height="1" width="1"></td>
				<td width="55"> 
<form:radiobutton path="specialTaxNotice" value="Yes" /> Yes
				</td>
				<td width="247">
<form:radiobutton path="specialTaxNotice" value="No" /> No
				</td>
				<ps:trackChanges name="csfForm" property="specialTaxNotice" />
			</tr>
...
<c:if test="${csfForm.checksMailedTo == 'TR'}">Trustee</c:if>
				...
				<ps:trackChanges name="csfForm" property="checksMailedTo" />
			</tr>
...
<c:if test="${userProfile.currentContract.bundledGaIndicator ==false}">
						 <td width="28%">
<form:radiobutton path="whoWillReviewWithdrawals" value="NR" /> No Review
						</td>
</c:if>
<c:if test="${csfForm.tpaFirmExists !=true}">
						<td>
<form:radiobutton path="whoWillReviewWithdrawals" value="PS" /> Plan Sponsor
						</td>
</c:if>
<c:if test="${csfForm.tpaFirmExists ==true}">
						<td width="15%">
<form:radiobutton path="whoWillReviewWithdrawals" value="TPA" /> TPA
						</td>
						<td width="57%">
<form:radiobutton path="whoWillReviewWithdrawals" value="PS" /> Plan Sponsor
						</td>
				...
				<ps:trackChanges name="csfForm" property="whoWillReviewWithdrawals" />
			</tr>	
	        ...
	        <% } %>
				...
				<td width="1" class="greyborder"><img src="/assets/unmanaged/images/spacer.gif" border="0" height="1" width="1"></td>
				<td width="60">
<form:radiobutton path="creatorMayApproveInd" value="Yes" /> Yes
				</td>
				<td width="242">
<form:radiobutton path="creatorMayApproveInd" value="No" /> No
				</td>
				<ps:trackChanges name="csfForm" property="creatorMayApproveInd" />
			</tr>
</c:if>
	</tbody>
</table>
