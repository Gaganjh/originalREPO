...
<script type="text/javascript">
		...
		function doStandardize(tpaFirmId) {
			...
			document.selectTpaFirmForPlanDataForm.submit();
		}
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<br> <ps:form action="/do/plandata/viewTpaPlanDataContractsView/"
				modelAttribute="selectTpaFirmForPlanDataForm"
				name="selectTpaFirmForPlanDataForm">
				<form:hidden path="selectedTpaFirmId" id="selectedTpaFirmId" />
				<%--  input - name="selectTpaFirmForPlanDataForm" --%>
			...
			<TABLE border=0 cellSpacing=0 cellPadding=0 width="403">
				<TBODY>
					...
					<c:if test="${not empty tpaFirmMap}">
						<c:forEach items="${tpaFirmMap}" var="theItem" varStatus="theIndex">
...
<%String temp = pageContext.getAttribute("indexValue").toString();%>
							...
							<TD class="payrollDate" align=left><a
								href="javascript:doStandardize(${theItem.key})">${theItem.key}
							</a></TD>
							<TD class="datadivider" width="1"><IMG
								src="/assets/unmanaged/images/s.gif" width=1 height=1></TD>
							<TD class="payrollDate" align=left><a
								href="javascript:doStandardize(${theItem.key})">${theItem.value}</a></TD>
							<TD class=databorder><IMG
								src="/assets/unmanaged/images/s.gif" width=1 height=1></TD>
							</TR>
						</c:forEach>
