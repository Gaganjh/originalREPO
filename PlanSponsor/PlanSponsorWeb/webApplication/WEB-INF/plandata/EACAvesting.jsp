...
<input type="hidden" name="tabPlanDataForm.excludeCount" class="excludeCount" value="${tabPlanDataForm.excludeCount}"/>
                                        ...
                                        <c:forEach items="${noticePlanCommonVO.vestingSchedules}"  var="vestingSchedule" varStatus="vestingScheduleStatus" >
                                         ...
                                         <tr class="${(vestingScheduleStatus.count % 2 == 0) ? 'evenDataRow' : 'oddDataRow'}  forIterate">
                                            <td class="textData" styleId="moneyTypeExcludeObject[${vestingScheduleStatus.count-1}].moneyTypeShortName" style="border-left-width: 0;">
                                            <ps:fieldHilight name="acVestingIncomplete${vestingScheduleStatus.count-1}" singleDisplay="true" className="errorIcon" displayToolTip="true"/>
...
<input type="hidden" name="tabPlanDataForm.noticePlanDataVO.noticePlanVestingMTExcludeVOList[${vestingScheduleStatus.count-1}].moneyTypeId" class="moneyTypeName" value="${vestingSchedule.moneyTypeShortName}"/>



                                            </td>
<input type="hidden" name="tabPlanDataForm.noticePlanDataVO.noticePlanVestingMTExcludeVOList[${vestingScheduleStatus.count-1}].excludeInd" class="excludeIndVal" id="excludeIndValue${vestingScheduleStatus.count-1}" value="tabPlanDataForm.noticePlanDataVO.noticePlanVestingMTExcludeVOList[${vestingScheduleStatus.count-1}].excludeInd"/>


                                          <!-- Code to get the exclude flag for the Money types -->
                                             <td class="textData" align ="center">

								<!-- Code to get the EACA exclude flag for the Money types -->	  	 	
									...
									<c:set var="isExcluded" value="false" />
									 <c:forEach
										items="${noticePlanDataVO.noticePlanVestingMTExcludeVOEacaList}"
										var="noticePlanVestingMTExcludeEaca"
										varStatus="vestingScheduleStatus1">
										
											<c:if
												test="${fn:trim(noticePlanVestingMTExcludeEaca. moneyTypeId) eq fn:trim(vestingSchedule.moneyTypeShortName)}">
												<input type="checkbox"
													id='moneyTypeExcludeCheckBox${vestingScheduleStatus.count-1}'
													onchange="setDirtyFlag();"
													name="eacaExcludedMoneyTypename"
													value='${fn:trim(vestingSchedule.moneyTypeShortName)}'
													Class="moneyTypeIDIndvalEaca" checked />
										 ...
										 <c:if test="${not isExcluded}">
											<input type="checkbox"
													...
													name="eacaExcludedMoneyTypename"
													value='${fn:trim(vestingSchedule.moneyTypeShortName)}'
													Class="moneyTypeIDIndvalEaca" />
										</c:if>		
									
								
								<!-- Code to get the EACA exclude flag for the Money types END-->
