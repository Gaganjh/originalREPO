...
Collection ADD_EDIT_USER_CONTRACT_LIST=(Collection)request.getAttribute(Constants.ADD_EDIT_USER_CONTRACT_LIST);
...
<content:contentBean
	contentId="<%=ContentConstants.SPECI_SIGN_WARN_MESS%>"
	type="<%=ContentConstants.TYPE_MISCELLANEOUS%>" beanName="specimensign" />
	

<input type="hidden" name="loginUserRole" /><%--  input - name="clientAddEditUserForm" --%>

<content:contentBean
	contentId="<%=ContentConstants.WARNING_ATLEAST_ONE_TRUSTEE_MAIL_RECIPIENT%>"
	type="<%=ContentConstants.TYPE_MESSAGE%>"
	id="warningAtleastOneTrusteeMailRecipient" />
<content:contentBean
	contentId="<%=ContentConstants.MORE_THAN_ONE_TRUSTEE_MAIL_RECIPIENT%>"
	type="<%=ContentConstants.TYPE_MESSAGE%>"
	id="moreThanOneTrusteeMailRecipient" />
<content:contentBean
	contentId="<%=ContentConstants.MORE_THAN_ONE_PPT_STATEMENT_CONSULTANT%>"
	type="<%=ContentConstants.TYPE_MESSAGE%>"
	id="moreThanOnePptStatementConsultant" />
<content:contentBean
	contentId="<%=ContentConstants.INVALID_ROLE_FOR_TRUSTEE_MAIL_RECIPIENT%>"
	type="<%=ContentConstants.TYPE_MESSAGE%>"
	id="invalidRoleForTrusteeMailRecipient" />
<content:contentBean
	contentId="<%=ContentConstants.WARNING_ICC_DESIGNATE_AVAILABLE%>"
	type="<%=ContentConstants.TYPE_MESSAGE%>"
	id="warningIccDesignateAvailable" />
<content:contentBean
	contentId="<%=ContentConstants.WARNING_SEND_SERVICE_DESIGNATE_AVAILABLE%>"
	type="<%=ContentConstants.TYPE_MESSAGE%>"
	id="warningSendServiceDesignateAvailable" />

<c:if test="${not empty ADD_EDIT_USER_CONTRACT_LIST}">
<c:set var="contractList" value="${ADD_EDIT_USER_CONTRACT_LIST}"/> 
</c:if>
<script type="text/javascript"
	src="/assets/unmanaged/javascript/layer_functions.js"></script>
<script  type="text/javascript">
...
function changeRole(obj){
	if (!submitted) {
		...
		var iccPermission = document.getElementsByName("contractAccesses[" + index + "].iccDesignate");
		var sendServicePermission = document.getElementsByName("contractAccesses[" + index + "].sendServiceDesignate");
		var doChange = false;
		var roleLabel = document.getElementsByName("contractAccesses[" + index + "].planSponsorSiteRole.label");
		var roleLabel1 = document.getElementsByName("contractAccesses[" + index + "].planSponsorSiteRole.label");
		...
		if (obj.options[roleOptionIndex].value == ""){
			...
			doChange = true;
		} else if((null != sendServicePermission) && (('PPY' == optionElement.value) || ('INC' == optionElement.value)) && (true == sendServicePermission.checked)){
			doChange = true;
		}  
		else {
...
function doSaveUser(){
	if (!submitted) {
var loginUserRole = "<%=clientAddEditUserForm.getLoginUserRole()%>";
...
<%if (request.getAttribute("duplicateSSNWarning") != null) {
				String warn = (String) request
						.getAttribute("duplicateSSNWarning");%>
function doOnload() {
	if(confirm("<%=warn%>")) {
		submitted = true;
...
<ps:form cssClass="margin-bottom:0;" method="POST" modelAttribute="clientAddEditUserForm" name="clientAddEditUserForm" 
	action="<%=actionPath%>">
...
<form:hidden path="staggingContactId"/>
	<table border="0" cellpadding="0" cellspacing="0" width="700">
		<tbody>
			<tr>
				<td width="475"><img src="/assets/unmanaged/images/s.gif"
					height="1" width="1"></td>
				<td width="5"><img src="/assets/unmanaged/images/s.gif"
					height="1" width="1"></td>
				<td width="170"><img src="/assets/unmanaged/images/s.gif"
					height="1" width="1"></td>
			</tr>
			<tr>
				<td><span class="highlight">*</span>Required Information <%-- error section --%>
					<p>
					<div id="errordivcs">
													...
													<table border="0" cellpadding="3" cellspacing="0"
														...
														<tbody>
															...
															<jsp:include flush="true"
																page="addEditProfileCommonSection.jsp" />
																</c:if>
																<c:if test="${userProfile.role.roleId eq 'PLC'}">
																<jsp:include flush="true"
																page="addEditProfileCommonSectionMobile.jsp" />
																
																</c:if> 
																<c:if test="${userProfile.role.roleId ne 'PLC'}">
															<tr class="datacell1">
																<td>
																	<%-- CEX8 SSN Label --%> <logicext:if
																		name="clientAddEditUserForm" property="webAccess"
																		op="equal" value="Yes">
																		<logicext:then>
																...
																<td colspan="2"><span class="bodytext"> <font
																		face="Verdana, Arial, Geneva, sans-serif" size="1">
																			<%--checking add page or edit page--%> <logicext:if
																				name="layoutBean" property="param(task)" op="equal"
																				value="add">
																				<%-- add page ssn empty --%>
																				<logicext:then>
																					...
																					<ps:trackChanges name="clientAddEditUserForm" property="ssn.digits[0]" />
																					<ps:trackChanges name="clientAddEditUserForm" property="ssn.digits[1]" />
																					<ps:trackChanges name="clientAddEditUserForm" property="ssn.digits[2]" />
																				</logicext:then>
																				...
																				<logicext:else>
																					...
																					<form:password path="ssn.digits[1]" showPassword="true"
																						cssClass="inputField" size="2" maxlength="2" />
																					<form:password path="ssn.digits[2]" showPassword="true"
																						autocomplete="off" cssClass="inputField" size="4" maxlength="4" />
																						</c:if>
																						<c:if test="${userProfile.role.roleId eq 'ICC'}">
																					<form:password path="ssn.digits[0]" showPassword="true"
																						cssClass="inputField" size="3" maxlength="3" />
															...
															<logicext:if name="clientAddEditUserForm"
																property="canManageAllContracts" op="equal" value="true">
																<logicext:then>
																	<tr class="datacell1">
																		<td valign="top"><strong>Web access<span
																				class="highlight">*</span>
																		</strong></td>
<td width="65" align="left" nowrap="nowrap"><form:radiobutton onclick="changeWebAccess();" path="webAccess" value="Yes"/>yes</td>


<td width="160" align="left" nowrap="nowrap"><form:radiobutton onclick="changeWebAccess();" path="webAccess" value="No"/>no<ps:trackChanges


																				name="clientAddEditUserForm" property="webAccess" /></td>
																	</tr>
																</logicext:then>
															</logicext:if>
															...
															<logicext:if name="layoutBean" property="param(task)"
																...
																<logicext:then>
																	...
																	<logicext:if name="clientAddEditUserForm"
																		property="webAccess" op="equal" value="Yes">
																		<logicext:then>
...
<td colspan="2" valign="top">${clientAddEditUserForm.userName}</td>
																			...
																			<tr class="datacell1">
																				<td valign="top"><strong>Password
																						status</strong></td>
																				<td colspan="2" valign="top"><ps:passwordState
																						name="clientAddEditUserForm"
															...
															<ps:isInternalUser name="userProfile" property="role">
																<tr class="datacell1">
																	<td valign="top"><ps:label fieldId="contractToAdd">Comments</ps:label>
																	</td>
<td colspan="2" valign="top"><form:textarea path="comments" cols="30" onblur="setMaxLength(this, maxCharForComments)" onkeyup="setMaxLength(this, maxCharForComments)" rows="3" />
													...
													changeTracker.trackElement('comments', document.clientAddEditUserForm.comments.value);
															...
															<ps:isExternal name="userProfile" property="role">
																...
																<c:if test="${not empty ADD_EDIT_USER_CONTRACT_LIST}">
																			...
																			<tr class="datacell1">
																				<td valign="top"><ps:label
																						fieldId="contractToAdd">Add contract number</ps:label></td>
<td colspan="2" valign="top"> <form:select path="contractToAdd">
															...
															<ps:isInternalUser name="userProfile" property="role">
																<ps:permissionAccess permissions="EXMN">
																	<tr class="datacell1">
																		<td valign="top"><ps:label
																				fieldId="contractToAdd">Add contract number</ps:label></td>
<td colspan="2" valign="top"><form:input path="contractToAdd" maxlength="<%=Constants.STR_CONTRACT_NUMBER_MAX_LENGTH%>" size="10" cssClass="inputField"/>
																...
																<logicext:then>
																	<tr class="datacell1">
																		...
																		<td colspan="2" valign="middle"
																			style="CURSOR: pointer" onclick="showRoleDef()">
																			<div align="right">
																				Role definitions&nbsp;<img
																					src="/assets/unmanaged/images/layer_icon.gif"
																					width="16" height="9">
																			</div>
																		</td>
																	</tr>
											...
											<jsp:include flush="true" page="contractAccessItem.jsp" />
																...
																<c:if test="${userProfile.role.roleId eq 'PLC'}">
																<%-- include contract access item --%>
											<jsp:include flush="true" page="contractAccessViewItemMobile.jsp" />
									...
									<table border="0" cellpadding="0" cellspacing="0" width="450">
										<tbody>
											<tr>
<td colspan="2" align="center"><input type="submit" class="button100Lg" onclick="return doCancelChanges(this.form);" name="action" value="<%=AddEditClientUserForm.BUTTON_LABEL_CANCEL%>" /> </td>




<td width="175" align="center"><input type="submit" class="button100Lg" onclick="return doSaveUser();" name="action" value="<%=AddEditClientUserForm.BUTTON_LABEL_SAVE%>" /> </td>



												<script type="text/javascript" >
											var onenter = new OnEnterSubmit('action', '<%=AddEditClientUserForm.BUTTON_LABEL_SAVE%>');
													onenter.install();
												</script>
											</tr>
										</tbody>
				...
				<td colspan="2" align="left" valign="top"><jsp:include
						flush="true" page="roledefinition.jsp" /></td>
			</tr>
		</tbody>
	</table>
</ps:form>
