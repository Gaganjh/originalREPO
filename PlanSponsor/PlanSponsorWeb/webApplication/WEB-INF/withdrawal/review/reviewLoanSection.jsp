          ...
          <c:forEach var="loan" 
            ...
            <tr class="datacell1" valign="top">
              <td class="shortSectionNameColumn">
                <a href="javascript:doLoanDetail(${loan.loanNo}, ${withdrawalForm.withdrawalRequestUi.employeeProfileId}, ${withdrawalForm.withdrawalRequestUi.contractId})">
                  Loan #${loan.loanNo}
                </a>
              </td>
          ...
          <tr class="datacell1" valign="top">
            ...
            <td class="longIndentedValueColumn">
                <ps:fieldHilight name="withdrawalRequestUi.withdrawalRequest.loanOption">
	                <ps:activityHistory itemNumber="${activityConstants.OUTSTANDING_LOANS_OPTION.id}"/>
                </ps:fieldHilight>
                <c:choose>
                	<c:when test="${withdrawalForm.withdrawalRequestUi.isParticipantInitiated or withdrawalForm.withdrawalRequestUi.viewOnly}">
                	<form:hidden path="withdrawalRequestUi.withdrawalRequest.loanOption" id="loanOptionId"/>

             			<ps:displayDescription collection="${loanTypes}" keyName="code" keyValue="description" 
				                  key="${withdrawalForm.withdrawalRequestUi.withdrawalRequest.loanOption}"/>
                	</c:when>
                	<c:otherwise>
            ...
            <tr class="datacell1" valign="top">
              ...
              <td>
                ...
                <ps:fieldHilight name="withdrawalRequestUi.withdrawalRequest.irsDistributionCodeLoanClosure">
                       <ps:activityHistory itemNumber="${activityConstants.IRS_DIST_CODE_FOR_LOANS.id}"/>
                    </ps:fieldHilight>
                  <c:choose>
                	<c:when test="${withdrawalForm.withdrawalRequestUi.viewOnly}">          
             			<ps:displayDescription collection="${irsDistCodesLoansTypes}" keyName="code" keyValue="description" 
				                  key="${withdrawalForm.withdrawalRequestUi.withdrawalRequest.irsDistributionCodeLoanClosure}"/>
                	</c:when>
                	<c:otherwise>
                    <form:select id="loanIrsDistributionCodeId"
                               cssClass="${(isPendingApproval or withdrawalForm.withdrawalRequestUi.mandatoryForBundledContract) ? 'mandatory' : ''}" 
