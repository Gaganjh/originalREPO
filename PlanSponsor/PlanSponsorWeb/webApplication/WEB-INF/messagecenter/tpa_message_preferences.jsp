...
function setSameTemplate(field,prefix) {
   ...
   for (var i=0; i < frm.length; i++) {
      ...
      if (fieldName.indexOf(prefix) >= 0) {
        ...
        if (tid == tid2 && field.value== frm.elements[i].value) {
...
<ps:form method="post" action="/do/messagecenter/tpaMessagePreferences"
	modelAttribute="messagePrefForm">

	<content:contentBean
		contentId="<%=MCContentConstants.ApplyToAllContracts%>"
		type="<%=ContentConstants.TYPE_MISCELLANEOUS%>"
		id="applyToAllContractsLabel" />

	<content:contentBean
		contentId="<%=MCContentConstants.MultiContractCopyLine1%>"
		type="<%=ContentConstants.TYPE_MISCELLANEOUS%>" id="copyLine1" />

	<content:contentBean
		contentId="<%=MCContentConstants.MultiContractCopyLine2%>"
		type="<%=ContentConstants.TYPE_MISCELLANEOUS%>" id="copyLine2" />
	...
	<table border="0" cellpadding="0" cellspacing="0" width="708">
		<tbody>
			<mc:personalizationTab active="message"
				global="<%=(Boolean) MCUtils.isInGlobalContext(request)%>" tpa="true"
				noticalertTab="<%=(Boolean) session.getAttribute(MCConstants.ALERT_NOITICE_PREFERENCE)%>"
				enableNoticalertTab="<%=(Boolean) session.getAttribute(MCConstants.ENABLE_ALERT_NOITICE_PREFERENCE)%>" />
			<tr>
				...
				<td width="699">
					<table border="0" cellpadding="1" cellspacing="0" width="710">
						<tbody>
							<tr height="25">
								<td width="521" valign="middle" class="tableheadTD"><a
									href="javascript:toggleAll(true)"><img
										src="/assets/unmanaged/images/plus_icon.gif" border="0" /></a> / <a
									href="javascript:toggleAll(false)"><img
										src="/assets/unmanaged/images/minus_icon.gif" border="0" /></a> <b>All</b>
								</td>
								<td width="1" valign="middle" class="dataheaddivider"><img
									src="/assets/unmanaged/images/spacer.gif" border="0" height="1"
									width="1"></td>
								<td width="100" valign="middle" class="tableheadTD"
									onmouseover="Tip(receiveHoverOver)" onmouseout="UnTip()">
									<b>Receive?</b>
								</td>
								<td width="1" valign="middle" class="dataheaddivider"><img
									src="/assets/unmanaged/images/spacer.gif" border="0" height="1"
									width="1"></td>
								<td width="75" valign="middle" class="tableheadTD"
									onmouseover="Tip(priorityHoverOver)" onmouseout="UnTip()">
									<b>Priority</b>
								</td>
							</tr>
						</tbody>
			...
			<tr>
				...
				<td>
					<!--  start1 --> <jsp:useBean id="messagePrefForm" scope="session"
						type="com.manulife.pension.ps.web.messagecenter.personalization.MCMessagePreferenceForm" />


					<c:forEach var="tab"
						items="${messagePrefForm.messageCenterTop.children}">
						...
						<c:if test="${messagePrefForm.tabContainsTemplate(tab)}">
							<table width="710" border="0" cellspacing="0" cellpadding="1">
								<tr height="30">
									<td class="tablesubMainhead"><span class="style1">${tab.name}</span></td>
								</tr>
							</table>
			...
			<tr>
				<td class="boxborder" colSpan="3"><IMG height="1"
		...
		<tr>
			<td width="385"><div align="right">
					<input class="button134" type="button" name="button1"
						value="cancel" id="cancelButton" disabled="disabled"
			...
			<td width="181">
				<div align="center">
					<input class="button134" type="button" name="button2" value="save"
						id="saveButton" disabled="disabled" onmouseover="Tip(saveLabel)"
						onmouseout="UnTip()" onclick="return doSave(this.form, 'save')">
				</div>
			</td>
			<td width="136">
				<div align="right">
					<input class="button134" type="button" name="button3"
						id="saveAndFinishButton" disabled="disabled"
						onmouseover="Tip(saveFinishLabel)" onmouseout="UnTip()"
						value="save &amp; finish"
						onclick="return doSave(this.form, 'finish')">
				</div>
			</td>
		</tr>
	</table>
