...
<c:set var="howtoId" value="${layoutBean.getParam('howto')}" /> 
	            	  ...
	            	  <% if ( SessionHelper.getUserProfile(request).isBundledGACAR()) { %>
	            	 	  <content:contentBean contentId="<%=MCContentConstants.Support%>"
			                               type="<%=ContentConstants.TYPE_MISCELLANEOUS%>"
			                               id="contentBean" override="true"/>
				          <tr>
				            <td width="17">
				            <% if (MCUtils.isInGlobalContext(request)){ %>
				            	<a href="<%=MCConstants.DispathUrlfromGlobal%>">
				            <%} else { %>
				            ...
				            <%} else { %>
				            	<a href="<%=MCConstants.DispatchUrl%>">
				             <% } %>
				           		<content:getAttribute id="contentBean" attribute="text"/></a>
				            </td>
				          </tr>  
				      <% } else{ %>
			          ...
			          <tr>
			            <td width="17">
						  <a href="<%=MCConstants.PersonalizationUrl%>">
						    <content:image id="contentBean" contentfile="image"/></a>
						</td>
			            <td width="5"><img src="/assets/unmanaged/images/s.gif" width="5" height="1"></td>
			            <td>
