...
<ps:form method="post" name="messagePrefForm" modelAttribute="messagePrefForm" action="/do/messagecenter/tpaMessagePreferences" >
...
<table border="0" cellpadding="0" cellspacing="0" width="708">
	<tbody>
		<mc:carViewsPersonalizationTab active="tpaPrefs"  tpa="<%=(Boolean)request.getAttribute(MCConstants.AttrUserIdTpa)%>" noticalertTab="<%= (Boolean)session.getAttribute(MCConstants.ALERT_NOITICE_PREFERENCE) %>" enableNoticalertTab="<%= (Boolean)session.getAttribute(MCConstants.ENABLE_ALERT_NOITICE_PREFERENCE) %>"/>
		<tr>
      ...
      <td width="136">
        <div align="right">
          <c:if test="${messagePrefForm.userCanEdit}">
            <input class="button134" type="button" name="button3" 
              id="edit" 
              value="edit" onclick="document.location='/do/mcCarView/editTpaMessagePreferences?userProfileId=<%=request.getParameter("userProfileId")%>&contractId=<%=request.getParameter("contractId") %>'" >
          </c:if>    
        </div>
