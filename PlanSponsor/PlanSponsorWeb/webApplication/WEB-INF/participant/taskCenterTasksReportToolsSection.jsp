...
<c:set var="temp" value= "${contentId}" />
	<c:choose>
	<c:when test="${isPayrollFeedbackServiceEnabled and temp eq '59221'}" >
		<!--  For Payroll Feedback No Need to display How to use this Page link -->
	</c:when>
	<c:otherwise>
	<%Integer contentId =(Integer)pageContext.getAttribute("temp"); %>
	 ...
	 %>
	          ...
	          <% if (contentId.intValue() == ContentConstants.COMMON_PRINT_REPORT_TEXT) { 
			    ...
			    //SSE 024 allow to download report, but mask SSN
				...
				String result = "/do/participant/taskCenterTasks?task=download&ext=" + java.net.URLEncoder.encode(extension);                    
		                    href = result;
		                    onclick = "return true;";	
		                    display = "true";
		     }  
		     else if ( (contentId.intValue() == 59221) ||
