...
<%Integer contentId =(Integer)pageContext.getAttribute("temp"); %>
 ...
 %>
          ...
          <% if (contentId.intValue() == ContentConstants.COMMON_PRINT_REPORT_TEXT) {
              	href = "javascript://";
                onclick = "doPrint()";
          } else if (contentId.intValue() == ContentConstants.COMMON_CENSUS_DOWNLOAD_TO_CSV_TEXT) { %>

			<%-- PPR 50.	User must have a specific permission to be able to download the census report. --%>
<c:if test="${participantAddressesReportForm.allowedToDownload ==true}">
	        <%
                	...
                	String result = "/do/participant/participantAddresses?task=download&ext=" + java.net.URLEncoder.encode(extension);
               	    href = result;
                    onclick = "return true;";
            %>
</c:if>
<c:if test="${participantAddressesReportForm.allowedToDownload ==false}">
		  	<%
 	            removeLink = "true";
 	        %>
</c:if>
...
<c:set var="howToReadReportKey" value="${howToReadReportLinkParamName}"/>
...
<c:if test="${empty howToReadReportLinkParamName1}">
          ...
          <% }else if (contentId.intValue() == ContentConstants.CENSUS_ADDRESS_DOWNLOAD_TO_CSV_TEXT) {
                ...
                String extension = null;
              	...
              	String result = "/do/participant/participantAddressSummary?task=download&ext=" + java.net.URLEncoder.encode(extension);
             	href = result;
                onclick = "return true;";
			}%>
