...
$(document).ready(function(){
    if (${passcodeTransitionForm.smsSwithOn}){
    	$('#sms').addClass("greytext");
    } 
    if (${passcodeTransitionForm.voiceSwithOn}){
    	$('#vmobile').addClass("greytext");
    	$('#vphone').addClass("greytext");
    } 
    if (${passcodeTransitionForm.emailSwithOn}){
    ...
    <tbody>
        <tr>
            ...
            <td width="50%" valign="top" class="greyText">
                ...
                <br>
                <ps:form method="POST" action="/do/passcodeTransition/" modelAttribute="passcodeTransitionForm" name="passcodeTransitionForm" onsubmit="return doPreSubmit();">
                    <c:choose>
                    <c:when test="${(not empty passcodeTransitionForm.maskedMobile) or (not empty passcodeTransitionForm.maskedPhone) }">
                        <br>
                		<br>
                <table width="426" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
                        ...
                        <c:if test="${(passcodeTransitionForm.smsSwithOn eq true) or (passcodeTransitionForm.voiceSwithOn eq true) or (passcodeTransitionForm.emailSwithOn eq true)}">
                        <tr>
                            <td width="5"><img src="/assets/unmanaged/images/s.gif" width="5" height="1"></td>
                            <td valign="top" width="421">
                            <content:contentBean
                                    ...
                                    <td colspan="2" align="center">
                                        <table>
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <c:if test="${not empty passcodeTransitionForm.maskedMobile}">
                                                        <div id = "sms">
                                                            <form:radiobutton disabled="${passcodeTransitionForm.smsSwithOn}" path="passcodeDeliveryPreference" value="SMS" />
                                                            <content:getAttribute attribute="body1" beanName="layoutPageBean">
                                                                <content:param>
                                                                    <c:out value="${passcodeTransitionForm.maskedMobile}" />
                                                                </content:param>
                                                            </content:getAttribute>
                                                        </div>    
                                                            <br>
                                                        <div id = "vmobile">
                                                            <form:radiobutton disabled="${passcodeTransitionForm.voiceSwithOn}" path="passcodeDeliveryPreference" value="VOICE_TO_MOBILE" />
                                                        ...
                                                        <c:if test="${not empty passcodeTransitionForm.maskedPhone}">
                                                        <div id = "vphone">
                                                            <form:radiobutton disabled="${passcodeTransitionForm.voiceSwithOn}" path="passcodeDeliveryPreference" value="VOICE_TO_PHONE" />
                                                            <content:getAttribute attribute="body3" beanName="layoutPageBean">
                                                                <content:param>
                                                                    <c:out value="${passcodeTransitionForm.maskedPhone}" />
                                                                </content:param>
                                                            </content:getAttribute>
                                                        </div>     
                                                            <br>
                                                        </c:if>
                                                        <c:if test="${not empty passcodeTransitionForm.maskedEmail}">
                                                        <div id = "email">
                                                        <content:contentBean contentId="96892" type="<%=ContentConstants.TYPE_MISCELLANEOUS%>" id="email" />
                                                            <form:radiobutton disabled="${passcodeTransitionForm.emailSwithOn}" path="passcodeDeliveryPreference" value="EMAIL" />
                                                            <content:getAttribute attribute="text" beanName="email">
                                                                <content:param>
                                                                    <c:out value="${passcodeTransitionForm.maskedEmail}" />
                                                                </content:param>
                                                            </content:getAttribute>
                                                        </div>   
                                                            <br>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <content:getAttribute attribute="introduction2" beanName="dynamicContent">
                                        ...
                                        <content:param>
                                           <b><c:out value="${passcodeTransitionForm.maskedEmail}" /></b>
                                        </content:param>
                                    </content:getAttribute>
                                    </td>
                                </tr>
