...
<strong>${submissionItem.submissionId}</strong></td>
	<td class="databorder"><img src="/assets/unmanaged/images/s.gif" width="1" height="1" /></td>
</tr>
<tr class="datacell1">
	<td class="databorder"><img src="/assets/unmanaged/images/s.gif" width="1" height="1" /></td>
	<% if (userProfile.isInternalUser()) { %>
<td class="datacell1" title="${submissionItem.systemStatus} <%-- ignore="true" --%> - <%=helper.getSystemStatusDescription(submissionItem)%>">
	<% } else { %>
		<td class="datacell1">
	<% } %>
		<b>Status:</b></td>
...
${submissionItem.submitterName} <%-- ignore="true" --%></strong></td>
</c:if>
<c:if test="${submissionItem.internalSubmitter ==true}">
...
<!--
<% if ( submissionItem.isLocked() ) {%>
<tr class="datacell1">
	<td class="databorder"><img src="/assets/unmanaged/images/s.gif" width="1" height="1" /></td>
	<td class="datacell1" class="submitterName"><b>In Use by:</b></td>
	<% if (userProfile.isInternalUser()) { %>
	<c:if test="${submissionItem.lockedByInternalUser!=true}">
		<td class="highlight" colspan="2" title="${submissionItem.lock.userId}">
		<strong>"${submissionItem.lock.userId}"</strong>
	</c:if>
	<c:if test="${submissionItem.lockedByInternalUser==true}">
		<td class="highlight" colspan="2" title="${submissionItem.lock.userName}">
		<strong>John Hancock Representative</strong>
	</c:if>
	<% } else { %>
	<c:if test="${submissionItem.lockedByInternalUser!=true}">
		<td class="highlight" colspan="2">
		<strong>${submissionItem.lock.userName}</strong>
	</c:if>
	<c:if test="${submissionItem.lockedByInternalUser==true}">
		<td class="highlight" colspan="2">
		<strong>John Hancock Representative</strong>
...
<tr class="datacell1">
	...
	<td class="highlight" valign="bottom" colspan="2">
<strong>${submissionItem.numberOfRecords}</strong>
	</td>
	<td class="databorder"><img src="/assets/unmanaged/images/s.gif" width="1" height="1" /></td>
</tr>
...
<tr>
  <td class="databorder" colspan="3" valign="bottom"><img src="/assets/unmanaged/images/s.gif" width="1" height="1" /></td>
</tr>
</table>
