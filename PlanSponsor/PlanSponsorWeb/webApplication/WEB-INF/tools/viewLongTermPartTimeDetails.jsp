...
<content:contentBean contentId="<%=ContentConstants.SUBMISSION_UPLOAD_PAYMENT_TEXT%>" type="<%=ContentConstants.TYPE_MISCELLANEOUS%>" beanName="fileUploadPaymentNote" />
...
<c:set var="downloadWarning">
	<content:getAttribute id="downloadReportWarning" attribute="text" />
</c:set>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="fixedTable">
	<ps:form action="/do/tools/viewLongTermPartTimeDetails/" modelAttribute="longTermPartTimeDetailsForm" name="longTermPartTimeDetailsForm" method="POST" onsubmit="return confirmSend();">
		<tr>
			<td width="15" class="big">&nbsp;</td>
			<!-- column 2 summary section -->
			<td width="350" height="20" valign="top"><jsp:include page="longTermPartTimeSummarySection.jsp" flush="true" /></td>
		...
		<tr>
			<td width="15" class="big">&nbsp;</td>
			<td colspan="3">
				<%-- START new LTPT details table --%> <br /> <br /> 
				<c:if test="${not empty param.printFriendly }">
					<c:if test="${systemStatus!=STATUS_COMPLETE}">
						<report:submissionErrors errors="<%=theReport%>" longTermPartTimeDetailsErrors="true" width="505" printFriendly="true" forceView="true" />
					</c:if>
					<c:if test="${systemStatus==STATUS_COMPLETE}">
						<report:submissionErrors errors="<%=theReport%>" longTermPartTimeDetailsErrors="true" width="505" printFriendly="true" forceView="false" />
					</c:if>
				</c:if> 
				<br /> <br />
				...
				<c:if test="${empty param.printFriendly }">
					...
					<table border="0" cellspacing="0" cellpadding="0" align="center" width="700">
						<tr>
							<td width="20%">&nbsp;</td>
							<% if ( longTermPartTimeDetailsHelper.isLongTermPartTimeReportEditable() ) {
							%>
							<td width="40%"><input type="submit" class="button100Lg" onclick="window.location.href='/do/tools/editLongTermPartTimeDetails/?subNo=${submissionItem.submissionId}'; return false;"
								value="edit" /></td>
							<% } else { %>
							<td width="40%">&nbsp;</td>
							<% } %>
							<td width="40%"><input type="submit" class="button175" onclick="window.location.href='/do/tools/submissionHistory/'; return false;"
								value="submission history" /></td>
							<td width="20%">&nbsp;</td>
						</tr>
						<tr>
							<td valign="top">&nbsp;</td>
							...
							<td valign="top"><span class="content">
								<content:getAttribute beanName="editButtonDescription" attribute="text" /></span>
							</td>
							<% } else { %>
							<td valign="top">&nbsp;</td>
							<% } %>
							<td valign="top"><span class="content">
								<content:getAttribute beanName="historyButtonDescription" attribute="text" /></span>
							</td>
							<td valign="top">&nbsp;</td>
						</tr>
					</table>
				</c:if> <%-- END new LTPT details table --%> <br>
			</td>
		</tr>
		<tr>
			<td class="big">&nbsp;</td>
		...
		<tr>
			...
			<td colspan="3">
				<p>
					<content:pageFooter id="layoutPageBean" />
				</p>
			</td>
		</tr>
	</ps:form>
