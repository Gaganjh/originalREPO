...
<jsp:useBean id="submissionHistoryForm"
                         scope="session" 
                         class="com.manulife.pension.ps.web.tools.SubmissionHistoryForm" />
...
function submitform()
{
	if(document.getElementsByName("justMine")[0].checked){
		setFilterFromInput(document.getElementsByName("justMine")[0]);
	}
	else {
		document.getElementsByName("justMine")[0].value = false;
		setFilterFromInput(document.getElementsByName("justMine")[0]);
	}
	<c:if test="${submissionHistoryForm.filterSimple != true}">
	...
	if(document.getElementsByName("filterBySubmission")[0].checked) {
		document.getElementsByName("filterBySubmission")[0].value = true;
		setFilterFromInput(document.getElementsByName("filterBySubmission")[0]);
	}
	</c:if>
    doFilter();
}
...
<% } %>
	...
	document.forms['submissionHistoryForm'].elements['task'].value = "sort";
	document.forms['submissionHistoryForm'].elements['sortField'].value = sortfield;
	document.forms['submissionHistoryForm'].elements['sortDirection'].value = sortDirection;
	document.forms['submissionHistoryForm'].submit();

}

function pagingSubmit(pageNumber){
	if (document.forms['submissionHistoryForm']) {

		document.forms['submissionHistoryForm'].elements['task'].value = "page";
		document.forms['submissionHistoryForm'].elements['pageNumber'].value = pageNumber;
...
<ps:form cssStyle="margin-bottom:0;"
         method="POST"
         modelAttribute="submissionHistoryForm" name="submissionHistoryForm"
         action="/do/tools/submissionHistory/">
<!--------------  body  ----------------------->
				...
				<tr class="tablehead">
					...
					<td colspan="4" align="right" class="tableheadTD">
						<span class="databorder">
							<report:pageCounterViaSubmit report="theReport" arrowColor="white" name="parameterMap" formName="submissionHistoryForm"/>
						</span>
					</td>
				...
				<tr>
					...
					<td height="51" colspan="10" class="datacell1">
						<table border="0" cellpadding="" cellspacing="4" bgcolor="#ffffff" class="datacell1">
							...
							<tr>
								<td colspan="2">
									...
									<form:select path="filterShowDate" onchange="setFilterFromSelect(this);">
										<form:options  path="datesCollection" items="${submissionHistoryForm.datesCollection}" itemLabel="label" itemValue="value"/>
									</form:select>
								</td>
								<td>
									<logicext:if name="<%= Constants.SHOW_JUST_MINE_FILTER %>" op="equal" value="true">
										...
										<logicext:else>
											&nbsp;
											<form:hidden path="justMine" value="true"/>
										</logicext:else>
									</logicext:if>
								</td>
								<td>
									<input type="button" onclick="javascript:submitform();" name="submitButton" class="button89x21" value="search"/>
								</td>
								<td>
									<a href="?filterSimple=false">Advanced Search</a><br>
									<c:if test="${submissionHistoryForm.filterActive == true}">
							...
							<tr>
								<td>
									<form:radiobutton path="filterBySubmission" value="true"/>Submission Date
<% if (userProfile.getCurrentContract().isDefinedBenefitContract())  { %>	
									<form:radiobutton path="filterBySubmission" value="false"/>Contribution Date
<% } else { %>								
									<form:radiobutton path="filterBySubmission" value="false"/>Payroll Date
<% } %>									
								</td>
								<td align="right">
									Submission Type
								...
								<td colspan="2">
									<form:select path="filterType" onchange="setFilterFromSelect(this);">
										<form:options items="${submissionHistoryForm.submissionTypes}" itemLabel="label" itemValue="value" />
									</form:select>
								</td>
							</tr>
							<tr>
								<td align="right">
									...
									<form:select path="filterStartDate" onchange="setFilterFromSelect(this);">
										<form:options items="${submissionHistoryForm.fromMonthsCollection}" itemLabel="label" itemValue="value"/>
									</form:select>
								</td>
								<td align="right">
									Submission Status
								...
								<td colspan="2">
									<form:select path="filterStatus" onchange="setFilterFromSelect(this);">
										<form:options items="${submissionHistoryForm.submissionStatuses}" itemLabel="label" itemValue="value"/>
									</form:select>
								</td>
							</tr>
							<tr>
								<td align="right">
									...
									<form:select path="filterEndDate" onchange="setFilterFromSelect(this);">
										<form:options items="${submissionHistoryForm.toMonthsCollection}" itemLabel="label" itemValue="value"/>
									</form:select>
								</td>
								<td>
									<logicext:if name="<%= Constants.SHOW_JUST_MINE_FILTER %>"  op="equal" value="true">
					...
					<td valign="top" align="left" class="submissionNumber">
						<b>
							<report:sortLinkViaSubmit formName="submissionHistoryForm" field="<%=SubmissionHistoryReportData.SORT_SUBMISSION_ID%>" direction="asc">Submission<br/>number</report:sortLinkViaSubmit>
						</b>
					</td>
					...
					<td valign="top" align="left" class="submissionDate">
						<b>
							<report:sortLinkViaSubmit formName="submissionHistoryForm" field="<%=SubmissionHistoryReportData.SORT_SUBMISSION_DATE%>" direction="desc">Submission<br/>date/time</report:sortLinkViaSubmit>
						</b>
					</td>
					<td class="dataheaddivider"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
					<td valign="top" align="left" class="type">
						<b><report:sortLinkViaSubmit formName="submissionHistoryForm" field="<%=SubmissionHistoryReportData.SORT_TYPE%>" direction="asc">Submission<br/>type</report:sortLinkViaSubmit></b>
					</td>
					<td class="dataheaddivider"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
					<td valign="top" align="left" class="payrollDate">
						<b>
<% if (userProfile.getCurrentContract().isDefinedBenefitContract())  { %>							
							<report:sortLinkViaSubmit formName="submissionHistoryForm" field="contribDate" direction="desc">Contribution date</report:sortLinkViaSubmit>
<% } else { %>
							<report:sortLinkViaSubmit formName="submissionHistoryForm" field="<%=SubmissionHistoryReportData.SORT_PAYROLL_DATE%>" direction="desc">Payroll date</report:sortLinkViaSubmit>
<% } %>							
						</b>
					</td>
					<td class="dataheaddivider"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
					...
					<td valign="top" align="left" class="submitterName">
						<b>
							<report:sortLinkViaSubmit formName="submissionHistoryForm" field="<%=SubmissionHistoryReportData.SORT_SUBMITTER_NAME%>" direction="asc">Submitted by</report:sortLinkViaSubmit>
						</b>
					</td>
					<td class="dataheaddivider"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
					<td valign="top" colspan="2" align="left" class="status">
						<b><report:sortLinkViaSubmit formName="submissionHistoryForm" field="<%=SubmissionHistoryReportData.SORT_USER_STATUS%>" direction="asc">Status</report:sortLinkViaSubmit></b>
					</td>
					<td class="databorder"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
				</tr>
...
<c:set var="indexValue" value="${theIndex.index}"/>
					...
					<%  SubmissionHistoryItem theRecord = (SubmissionHistoryItem)(pageContext.getAttribute("theItem"));
				...
				<tr class="datacell2">
					...
					<%
						...
						boolean viewAllowed = helper.isViewAllowed(theRecord,userProfile);
					%>
							<td class="databorder"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
...
${theItem.submissionId}
							...
							<td align="left" class="submissionDate">
								<% if (theRecord.isDraft()) { %>
									not submitted
								<% } else if (theRecord.isFileLoaderCase()) { %>
							...
							<td align="left" class="type">
								<%
									if ( helper != null ) {
								%>
									<%= helper.getDisplayType(theRecord) %>
								<% } %>
							</td>
							<td class="datadivider"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
...
<c:if test="${theItem.type !='E'}">
											<% if ( ( "C".equals(theRecord.getType()) || "X".equals(theRecord.getType()) ) && theRecord.getPayrollDate() == null ) { %>
												Not Available
											<% } else { %>
...
<c:if test="${theItem.type !='Q'}">
												<% if (theRecord.getContributionTotal() == null) { %>
													Not Available
												<% } else { %>
...
<c:if test="${theItem.type !='Q'}">
											...
											<% } %>
											<% if (theRecord.getPaymentTotal() != null) { %>
												<% if (theRecord.isZeroContribution() || Constants.BIG_DECIMAL_ZERO.compareTo(theRecord.getPaymentTotal()) == 0 ) { %>
													Not available
												<% } else { %>
							...
							<% if (userProfile.isInternalUser()) { %>
								 <% if ((("I".equals(theRecord.getSubmitterType()) && "FL".equals(theRecord.getApplicationCode()))
									        || theRecord.isInternalSubmitter())) { %>
...
<td align="left" class="submitterName" title='${theItem.submitterID}' <%-- ignore="true" --%>&nbsp;&nbsp;&nbsp;">
							...
							<% } else if ( ("I".equals(theRecord.getSubmitterType())) && ("AL".equals(theRecord.getApplicationCode()))) {	%>
...
${theItem.submitterName} <%-- ignore="true" --%>
							...
							<td class="datadivider"><img src="/assets/unmanaged/images/s.gif" width="1" height="1"></td>
						<% if (userProfile.isInternalUser()) { %>
<td colspan="2" align="left" class="status" title='${theItem.systemStatus}' <%-- ignore="true" --%> - <%=helper.getSystemStatusDescription(theRecord)%>'>
						<% } else { %>
							<td colspan="2" align="left" class="status">
						<% } %>
						<%
							if ( helper != null ) {
								if (helper.getDisplayStatus(theRecord).startsWith("Complete*")) {
									completeMessage = "true";
								}
						%>
								
	                        <!--to get Awaiting Payment -->
							<% if(theRecord.getSystemStatus().equals(Constants.AWAITING_PAYMENT_STATUS)){%>
							<report:actions profile="userProfile" item="theItem" action="view"/>
							<%}%>
	                        
										<%= helper.getDisplayStatus(theRecord) %>							
...
<c:if test="${empty theReport.details}">
				...
				<tr>
					<td colspan="17" align="right">
						<report:pageCounterViaSubmit report="theReport" arrowColor="black" name="parameterMap" formName="submissionHistoryForm"/>
					</td>
				</tr>
			</table>
		</td>
