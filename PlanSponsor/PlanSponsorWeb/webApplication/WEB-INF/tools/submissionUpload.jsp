...
// lastpayroll => lastPayroll
	...
	function isFormChanged() {
		...
		if (getFileType(uploadFormObj) != null) {
			return true;
		}
...
cashAccountRow = accountsRow${theIndex.index};
	...
	function getFileType(uploadFormObj) {
		...
		if (uploadFormObj.fileType[0].checked)
			return uploadFormObj.fileType[0].value;
		if (uploadFormObj.fileType[1].checked)
			return uploadFormObj.fileType[1].value;
...
-->
	...
	<A NAME="TopOfPage"></A>&nbsp;
          <table width="100%" border="0" cellpadding="0" cellspacing="0" class="fixedTable">
				...
				<td>
					<DIV id="inProgressDivision" style="display: none;";>
						<TABLE BORDER=0 CELLPADDING=1 CELLSPACING=0>
							...
							<br>If you have trouble with your upload click <a href="/do/tools/submissionUpload/">here</a> to start over.
							</TD>
							</TR>
						</TABLE>
					</DIV>
            ...
            <tr>
            <td height="20">
    	 		   <ps:form action="/do/tools/submissionUpload/" enctype="multipart/form-data" method="POST" onsubmit="return confirmSend();" modelAttribute="submissionUploadForm" name="submissionUploadForm">
									...
									<tbody>
									  <tr>
										...
										<td width="178" align="left" valign="top" class="datacell1">
										  <span class="content">
<form:radiobutton onclick="selectFileType('C')" path="fileType" value="C"/>
										  Regular Contribution</span>
										  <br/>
<form:radiobutton onclick="selectFileType('X')" path="fileType" value="X"/>
										  Transfer Contribution</span>
										</td>
...
<script>
	...
	if (fileTypeObjs.length > 0) {
		if (fileTypeObjs[0].checked) {
			disableLastquaterSelection(false);
		} else {
			disableLastquaterSelection(true);
		}
