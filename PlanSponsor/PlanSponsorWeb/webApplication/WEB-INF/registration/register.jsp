...
$(document).ready(function(event) {
			...
			$("#password").on("keyup",function(e) {
			 if(document.getElementById('confirmPassword').value == document.getElementById('password').value && pwdStat== true){
				document.getElementById("continue").disabled = false;
				document.getElementById("continue").style.color = "";
			 }
			 ...
			 if(keyCodeStatus != 16) {
				...
				//The below method does Client validation first and then server validation
				 doPasswordValidation(document.getElementById("password").value,document.getElementById("userName").value);
			}
    });
				
			//confirmPassword validation
			$("#confirmPassword").on("keyup",function(e) {
					 var newPasswordStr = document.getElementById('password').value;
...
var deApiStatus;
 function doServerValidation(csrfToken){
		...
		var resultToken ;
		var jsonItem = JSON.stringify({newPassword : document.getElementById("password").value,userName : document.getElementById("userName").value});
		var http = new XMLHttpRequest();
		var url = "/do/registration/register/ajaxvalidator/?"+"_csrf="+csrfToken;
	     ...
	     if(http.readyState == 4 && http.status == 200) {
	       try{
			...
			if(resultToken >=0)
			{
				...
				if(document.getElementById("password").value!="" && isContextSpecificWordsIND != true)
				{
					$('span.seven').remove();
					$('#seventhmsg').prepend('<span class ="seven" style="color: green; font-size: 12px;"> &#10004;</span>');
				}else{
	...
	<tbody>
		<tr>
			...
			<td width="50%" valign="top" class="greyText"> <img src="/assets/unmanaged/images/s.gif" width="402" height="23"><br>
				...
				<ps:form method="POST" modelAttribute="registerForm" name="registerForm" action="/do/registration/register/" >
...
${registerForm.lastName}
						...
						<tr valign="top" class="datacell1">
							<td><ps:label fieldId="emailAddress" mandatory="true">Email</ps:label></td>
							<td>
<form:input path="email" size="40" cssClass="inputField"/>
					...
					<tr class="datacell1">
						...
						<tr valign="top" class="datacell1">
							<td colspan="2">
								<ps:label fieldId="passcodeDeliveryPreference" mandatory="true">
									<content:contentBean
										contentId="<%=ContentConstants.CHANNEL_CHOICE%>"
										type="<%=ContentConstants.TYPE_MISCELLANEOUS%>"
										beanName="channelSelection" />
									<content:getAttribute beanName="channelSelection"
										attribute="text" />
								</ps:label>
							</td>
						</tr>
						...
						<tr valign="top" class="datacell1">
							...
							<td>
								<form:password path="password" id="password" cssClass="inputField" size="28" maxlength="64" value="${registerForm.password}"/>
						...
						<tr valign="top" class="datacell1">
							...
							<td>
								<form:password path="confirmPassword" id="confirmPassword" cssClass="inputField" size="28" maxlength="64" value="${registerForm.confirmPassword}"/>
							</td>
						</tr>
						...
						<tr valign="top" class="datacell1">
							<td><ps:label fieldId="challengeQuestion" mandatory="true">Challenge question</ps:label></td>
							<td>
<form:input path="challengeQuestion" maxlength="100" size="40" cssClass="inputField"/>
							</td>
						</tr>
						<tr valign="top" class="datacell1">
							<td><ps:label fieldId="answer" mandatory="true">Answer</ps:label></td>
							<td>
								<form:password path="answer" id="answer" cssClass="inputField" size="40" maxlength="32" value="${registerForm.answer}"/>
							</td>
						</tr>
						<tr valign="top" class="datacell1">
							<td><ps:label fieldId="confirmAnswer" mandatory="true">Confirm answer</ps:label></td>
							<td>
								<form:password path="confirmAnswer" id="confirmAnswer" cssClass="inputField" size="40" maxlength="32" value="${registerForm.confirmAnswer}"/>
							</td>
						</tr>
					...
					<tbody><tr align="center">
						...
						<td width="139">&nbsp;</td>
						<td width="144">
<input type="submit" class="button100Lg" id="continue" onclick="javascript:setButtonAndSubmit('continue');return false;" value="continue"/>
						</td>
					</tr>
					</tbody>
					</table>
