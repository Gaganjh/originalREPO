...
function registerTrackChangesFunction(func, tab, warningMsg) {
...
function discardChanges() {
  ...
  if (formChanged) {
    if (window.confirm(cancelWarning)) {
    	var request ;
    	// send a AJAX request to release lock for comment record
		if(tabName == 'contactsPS') {
			YAHOO.util.Connect.setForm(document.planSponsorContactsReportForm);
			actionPath = "/do/contacts/planSponsor/?task=cancelContractComments";
			request = YAHOO.util.Connect.asyncRequest('GET', actionPath);
		} else if (tabName == 'contactsTPA') {
			YAHOO.util.Connect.setForm(document.tpaContactsReportForm);
			actionPath = "/do/contacts/thirdPartyAdministrator/?task=editCommentsCancel";
			request = YAHOO.util.Connect.asyncRequest('GET', actionPath);
		}
	...
	if (hrefs != null) {
		for (i = 0; i < hrefs.length; i++) {
			if (hrefs[i].onclick != undefined
					...
					|| hrefs[i].onclick.toString().indexOf("gotoUrl") != -1
					|| hrefs[i].onclick.toString().indexOf("doPrint") != -1
					|| hrefs[i].onclick.toString().indexOf("mailto") != -1)
			    ) {
