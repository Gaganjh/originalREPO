...
function searchButtonClicked() {
	var urlParams = enumerateFilters();
		...
		J.getJSON("/do/jhNavigator?proxyURL=0&keyWord="+searchq+urlParams+
				 "&maxResults=" + numResults + "&keywordType=" + keywordType+ "&psw=1",
				function(data) {
					parseResults(data);
				});
		J.get("/do/jhNavigator?proxyURL=2&keyWord="+searchq+urlParams+"&maxResults="
				+ numResults + "&keywordType=" + keywordType+ "&psw=1", function(data) {
			parseListResults(data);
		});
	}
	...
	function parseResults(jsonResults) {
		...
		if (jsonResults.numResults == 1) {
			document.getElementById('numResults').innerHTML = " "
					+ jsonResults.numResults + " Result returned";
		} else {
			document.getElementById('numResults').innerHTML = " "
					+ jsonResults.numResults + " Results returned";
		}
		J
				.each(
						jsonResults.items,
						function(i, item) {
							J("<img/>")
									.attr(
											{
												src : "https://www.jhnavigator.com/managed_assets/bd_thumbs/"
														+ item.thumb,
												height : 133,
												width : 100,
												border : 0,
												alt : ""
											}).appendTo("#thumbImgs").wrap(
											"<div id='thumbnail'><a onClick='javascript: openItem("
													+ item.itemVersionID
													+ ");'></a><br>"
													+ item.name + "</div>");
						})
	...
	function openItem(id) {
		vURL = '/do/jhNavigator?proxyURL=1&psw=1&itemVersionID=' + id;
		window
				.open(
						vURL,
						'Details',
						'left=150,top=150,height=580,width=730,resizable=no,scrollbars=yes,toolbar=no,location=no,directories=no');
	}
	...
	function generalFilterClicked()
	{
		...
		if (primeFlag == 0)
		{
			...
			var myResult = myString.replace(myRegExp, ",");
			searchq = encodeURI(myResult);
			J.getJSON("/do/jhNavigator?proxyURL=0&keyWord="+searchq+urlParams+"&maxResults="+numResults+"&keywordType="+keywordType+ "&psw=1", function(data){parseResults(data);});
			J.get("/do/jhNavigator?proxyURL=2&keyWord="+searchq+urlParams+"&maxResults="+numResults+"&keywordType="+keywordType+ "&psw=1", function(data){parseListResults(data);});
		}
	}
	...
	function enumerateFilters()
	{
		...
		var srchParams = "&language="+languageList;
		return srchParams;
	}
