 ...
 */
public class OldPasswordRule extends ValidationRuleSet {

	private static final OldPasswordRule instance = new OldPasswordRule();

	/**
	 * Constructor.
	 */
	public OldPasswordRule() {
		super();
		...
		addRule(new RegularExpressionRule(
				CommonErrorCodes.PASSWORD_FAILS_STANDARDS, "^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,64}$"),
				false);*/
	...
	public static OldPasswordRule getInstance() {
		return instance;
	}
