...
public class OptionsTag extends TagSupport {
    ...
    public int doEndTag() throws JspException {
        ...
        if (selectTag == null) {
            throw new JspException(BaseBundle.getMessage("optionsTag.select"));
        }
    ...
    protected Iterator getIterator(String name, String property)
        ...
        if (bean == null) {
            throw new JspException(BaseBundle.getMessage("getter.bean", beanName));
        }
        ...
        if (property != null) {
            try {
                ...
                throw new JspException(BaseBundle.getMessage("getter.access",
                        property, name));
            } catch (InvocationTargetException e) {
                Throwable t = e.getTargetException();

                throw new JspException(BaseBundle.getMessage("getter.result",
                        property, t.toString()));
            } catch (NoSuchMethodException e) {
                throw new JspException(BaseBundle.getMessage("getter.method",
                        property, name));
            }
        }
        ...
        if (collection instanceof Collection) {
        	...
        	EnumerationToIterator<String> enumToIt = new EnumerationToIterator<String>((Enumeration)collection);
        	return enumToIt;
        }else {
            throw new JspException(BaseBundle.getMessage("optionsTag.iterator",
                    collection.toString()));
        }
    }
}
