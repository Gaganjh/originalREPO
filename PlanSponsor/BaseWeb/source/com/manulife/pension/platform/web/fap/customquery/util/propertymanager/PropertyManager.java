...
/**
 * Copied from the FundQueryTool module. 
 * 
 * TODO: Need to remove the unwanted methods.
 * 
 * @author ayyalsa
 */
...
public class PropertyManager {
    ...
    public static final String CUSTOM_QUERY_PROP_FILE_KEY = "custom_query.filename";
	private static final String CUSTOM_QUERY_PROP_FILE = "FandpCustomQuery.properties";
...
//	private final Map propertyTypes = new HashMap();
	...
	public void resetDefaults() {		
		...
		//load fund_query_tool.properties
		String fundQueryToolPropertyFile = (String)getObjectValue(CUSTOM_QUERY_PROP_FILE_KEY, CUSTOM_QUERY_PROP_FILE);
		try {		
			types.add(new FilePropertyType(fundQueryToolPropertyFile));
		} catch(IOException x) {
			throw new IllegalArgumentException("PropertyManager. Problems loading File properties:" + fundQueryToolPropertyFile + " Exception:" + x.getMessage());
		}
	}
	...
	private Object replaceEmbeddedProperties(Object value) {
		...
		while(i>=0) {
			...
			String embeddedPropertyValue = null;
			try {
				embeddedPropertyValue = PropertyManager.getString(embeddedProperty);
				buff.append(embeddedPropertyValue);
			} catch (Exception e) {
				buff.append(prefix+embeddedProperty+suffix); //not found, leave as is
			}
	...
	public static Connection getConnection() throws SQLException {
		...
		String databaseUrl = PropertyManager.getString("database.url", null);
		String userId = PropertyManager.getString("database.user", null);
		String password = PropertyManager.getString("database.password", null);
		String databaseName = PropertyManager.getString("database.name", null);
