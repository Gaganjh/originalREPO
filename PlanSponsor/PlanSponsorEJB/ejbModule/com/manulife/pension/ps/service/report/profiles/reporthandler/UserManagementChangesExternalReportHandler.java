 ...
 *
 */
public class UserManagementChangesExternalReportHandler implements ReportHandler {
	
	public static final String REPORT_ID = UserManagementChangesExternalReportHandler.class.getName(); 
	...
	public ReportData getReportData(ReportCriteria reportCriteria)
		...
		UserManagementChangesExternalReportData reportData = new UserManagementChangesExternalReportData();
		reportData.setReportCriteria(reportCriteria);
		
		String actionCode = (String)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_ACTION );
		SecurityChangeHistoryDAO.ActionCode action = SecurityChangeHistoryDAO.ActionCode.getAction(actionCode);
		String teamCode = (String)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_TEAM_CODE );
        String changedByTeamCode = (String)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_CHANGED_BY_TEAM_CODE );
		Integer contractNumber = (Integer)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_CONTRACT_NUMBER );
		String startDateStr = (String)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_START_DATE );
		String endDateStr = (String)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_END_DATE );
		String lastName = (String)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_LAST_NAME );
		String userRoles = (String)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_ROLES );
		Boolean isExternalReport = (Boolean)reportCriteria.getFilterValue( UserManagementChangesExternalReportData.FILTER_USER_TYPE );
		String orderBy = ""; 
        ...
        String userType = (isExternalReport.booleanValue() ? "EU" : "IU");
