...
import com.manulife.pension.ps.service.report.participant.payrollselfservice.dao.PayrollSelfServiceDeferralDAO;
...
public class PayrollSelfServiceChangesReportHandler implements ReportHandler {
	...
	public ReportData getReportData(ReportCriteria criteria) throws SystemException {
		...
		if(PayrollSelfServiceChangeViewRecordType.ALL.getCode().equalsIgnoreCase(recordType)
			...
			ReportData reportData = new PayrollSelfServiceEnrollmentDAO().getReportData(criteria);
			if(reportData != null 
		...
		if (PayrollSelfServiceChangeViewRecordType.ALL.getCode().equalsIgnoreCase(recordType)
			...
			ReportData reportData = new PayrollSelfServiceDeferralDAO().getReportData(criteria);
			if (reportData != null 
					&& CollectionUtils.isNotEmpty(reportData.getDetails())) {
				mergedReportDetails.addAll(reportData.getDetails());
			}
